...
Provider-level CPB-local mixed gausslet/GTO kinetic pilot for one supplement
orbital. This reuses existing QW polynomial-Gaussian kinetic axis-integral
helpers in the separable sum Kx Sy Sz + Sx Ky Sz + Sx Sy Kz and does not imply
route/global placement, Hamiltonian assembly, or broader GTO operator support.
"""
struct CPBMixedGTOKineticLocalBlock{P,C,O,D,M}
    parent::P
    cpb::C
    orbital::O
    dense_block::D
...
Provider-level CPB-local mixed gausslet/GTO one-body block for a whole
Cartesian Gaussian shell supplement representation. This is a thin collection
wrapper around the one-orbital mixed GTO pilots and does not imply route/global
placement, Hamiltonian assembly, or WL/PQS realization.
"""
...
Provider-level CPB-local mixed gausslet/GTO nuclear-attraction block for one
center and a whole Cartesian Gaussian shell supplement. The center contribution
is kept separate for later counterpoise or Hamiltonian policy and is not route
or globally placed.
"""
struct CPBMixedGTOSupplementNuclearByCenterBlock{P,C,S,E,R,D,M}
    parent::P
    cpb::C
    supplement::S
    expansion::E
...
Provider-level GTO/GTO nuclear-attraction Galerkin block for one center and a
whole Cartesian Gaussian shell supplement. The block follows the existing
QW/GTO by-center convention and does not sum centers or assemble a Hamiltonian.
"""
struct CPBGTOSupplementNuclearByCenterBlock{S,E,R,D,M}
    supplement::S
    expansion::E
    center_record::R
    dense_block::D
...
function cpb_mixed_gto_overlap_block(
    ...
    _is_cartesian_gaussian_shell_supplement(orbital) && return _cpb_mixed_gto_supplement_block(
        parent,
        cpb,
        orbital;
        term = :mixed_gto_overlap,
        source_kind = :mixed_gausslet_gto_whole_supplement_overlap,
        available_status = :materialized_cpb_mixed_gto_supplement_local_overlap_block,
        blocked_status = :blocked_cpb_mixed_gto_supplement_local_overlap_block,
        block_builder = (parent, cpb, orbital) ->
            cpb_mixed_gto_overlap_block(parent, cpb, orbital),
    )
...
function cpb_mixed_gto_position_operator_block(
    ...
    _is_cartesian_gaussian_shell_supplement(orbital) && return _cpb_mixed_gto_supplement_block(
        ...
        source_kind = :mixed_gausslet_gto_whole_supplement_coordinate_moment,
        available_status =
            :materialized_cpb_mixed_gto_supplement_coordinate_moment_local_block,
        blocked_status =
            :blocked_cpb_mixed_gto_supplement_coordinate_moment_local_block,
        active_axis = axis,
        coordinate_moment = :position,
        xpower = 1,
        block_builder = (parent, cpb, orbital) ->
            cpb_mixed_gto_position_operator_block(parent, cpb, orbital; axis),
    )
...
function cpb_mixed_gto_x2_operator_block(
    parent::CPGB.CartesianParentGaussletBasis3D,
    cpb::CPB.CoordinateProductBox,
    orbital;
    axis::Symbol = :x,
    ...
    _is_cartesian_gaussian_shell_supplement(orbital) && return _cpb_mixed_gto_supplement_block(
        ...
        cpb,
        orbital;
        term = Symbol("mixed_gto_x2_", String(axis)),
        source_kind = :mixed_gausslet_gto_whole_supplement_coordinate_moment,
        available_status =
            :materialized_cpb_mixed_gto_supplement_coordinate_moment_local_block,
        blocked_status =
        ...
        xpower = 2,
        block_builder = (parent, cpb, orbital) ->
            cpb_mixed_gto_x2_operator_block(parent, cpb, orbital; axis),
    )
    blocker = _cpb_mixed_gto_coordinate_moment_block_blocker(
        parent,
        cpb,
...
function cpb_mixed_gto_kinetic_operator_block(
    parent::CPGB.CartesianParentGaussletBasis3D,
    cpb::CPB.CoordinateProductBox,
    orbital,
)
    _is_cartesian_gaussian_shell_supplement(orbital) && return _cpb_mixed_gto_supplement_block(
        ...
        source_kind = :mixed_gausslet_gto_whole_supplement_kinetic,
        available_status = :materialized_cpb_mixed_gto_supplement_kinetic_local_block,
        blocked_status = :blocked_cpb_mixed_gto_supplement_kinetic_local_block,
        block_builder = (parent, cpb, orbital) ->
            cpb_mixed_gto_kinetic_operator_block(parent, cpb, orbital),
    )
    blocker = _cpb_mixed_gto_overlap_block_blocker(parent, cpb, orbital)
    dense_block =
        isnothing(blocker) ?
        _materialize_cpb_mixed_gto_kinetic_block(parent, cpb, orbital) :
        nothing
    status =
        isnothing(blocker) ?
        :materialized_cpb_mixed_gto_kinetic_local_block :
        :blocked_cpb_mixed_gto_kinetic_local_block
    return CPBMixedGTOKineticLocalBlock(
        parent,
        cpb,
        orbital,
        dense_block,
        _cpb_mixed_gto_kinetic_block_summary(
            status,
            blocker,
            parent,
            cpb,
...
function cpb_mixed_gto_nuclear_by_center_block(
    parent::CPGB.CartesianParentGaussletBasis3D,
    cpb::CPB.CoordinateProductBox,
    supplement,
    expansion,
    ...
    blocker = _cpb_mixed_gto_nuclear_by_center_blocker(
        parent,
        cpb,
        supplement,
        expansion,
    ...
    dense_block =
        isnothing(blocker) ?
        _materialize_cpb_mixed_gto_nuclear_by_center_block(
            parent,
            cpb,
            supplement,
            expansion,
    ...
    return CPBMixedGTOSupplementNuclearByCenterBlock(
        parent,
        cpb,
        supplement,
        expansion,
        center_record,
        dense_block,
        _cpb_mixed_gto_nuclear_by_center_block_summary(
            status,
            blocker,
            parent,
            cpb,
...
function cpb_gto_nuclear_by_center_block(supplement, expansion, center_record)
    center_summary = _cpb_electron_nuclear_center_summary(center_record)
    blocker = _cpb_gto_nuclear_by_center_blocker(
        supplement,
        expansion,
        center_summary,
    )
    dense_block =
        isnothing(blocker) ?
        _materialize_cpb_gto_nuclear_by_center_block(
            supplement,
            expansion,
            center_summary,
        ) :
    ...
    return CPBGTOSupplementNuclearByCenterBlock(
        supplement,
        expansion,
        center_record,
        dense_block,
        _cpb_gto_nuclear_by_center_block_summary(
            status,
            blocker,
            supplement,
            expansion,
...
function _cpb_mixed_gto_supplement_block(
    ...
    orbitals = supplement.orbitals
    orbital_count = length(orbitals)
    if orbital_count == 0
        return CPBMixedGTOSupplementLocalBlock(
            parent,
            cpb,
            supplement,
            nothing,
            _cpb_mixed_gto_supplement_block_summary(
                blocked_status,
                :empty_mixed_gto_supplement_orbitals,
                parent,
                cpb,
                supplement,
                (),
    ...
    orbital_blocks = [block_builder(parent, cpb, orbital) for orbital in orbitals]
    orbital_summaries = Tuple(summary(block) for block in orbital_blocks)
    blocked_index = findfirst(
        orbital_summary -> orbital_summary.dense_block_available != true,
        orbital_summaries,
    )
    blocker = isnothing(blocked_index) ?
        nothing :
        :mixed_gto_supplement_orbital_block_blocked
    dense_block = if isnothing(blocker)
        reduce(hcat, (block.dense_block for block in orbital_blocks))
    else
        nothing
    end
...
function _cpb_gto_nuclear_expansion_blocker(expansion)
    ...
    isempty(coefficients) && return :empty_coulomb_expansion_coefficients
    isempty(exponents) && return :empty_coulomb_expansion_exponents
    length(coefficients) == length(exponents) ||
        return :coulomb_expansion_term_count_mismatch
    return nothing
end

function _cpb_mixed_gto_nuclear_by_center_blocker(
    parent::CPGB.CartesianParentGaussletBasis3D,
    cpb::CPB.CoordinateProductBox,
    supplement,
    expansion,
    ...
    expansion_blocker = _cpb_gto_nuclear_expansion_blocker(expansion)
    isnothing(expansion_blocker) || return expansion_blocker
    supplement_blocker = _cpb_gto_supplement_one_body_blocker(supplement)
    isnothing(supplement_blocker) || return supplement_blocker
    CPB.support_count(cpb) > 0 || return :empty_cpb_support
    parent_counts = CPGB.parent_axis_counts(parent)
    intervals = CPB.intervals(cpb)
    for (axis_index, interval) in pairs(intervals)
        first(interval) >= 1 || return :cpb_interval_outside_parent
        last(interval) <= parent_counts[axis_index] ||
            return :cpb_interval_outside_parent
    end
    return nothing
end

function _cpb_gto_nuclear_by_center_blocker(supplement, expansion, center_summary)
    center_summary.status == :available_electron_nuclear_center_record ||
        return center_summary.blocker
    expansion_blocker = _cpb_gto_nuclear_expansion_blocker(expansion)
    isnothing(expansion_blocker) || return expansion_blocker
    return _cpb_gto_supplement_one_body_blocker(supplement)
end

function _cpb_mixed_gto_nuclear_proxy_layers(
    parent::CPGB.CartesianParentGaussletBasis3D,
    expansion,
)
    axes = CPGB.parent_axes(parent)
    exponents = expansion.exponents
    proxy_helper = getproperty(parentmodule(@__MODULE__), :_qwrg_mapped_supplement_proxy_layer)
    return (;
        x = _cpb_mixed_gto_nuclear_proxy_layer(proxy_helper, axes.x, exponents),
        y = _cpb_mixed_gto_nuclear_proxy_layer(proxy_helper, axes.y, exponents),
        z = _cpb_mixed_gto_nuclear_proxy_layer(proxy_helper, axes.z, exponents),
    )
end

function _cpb_mixed_gto_nuclear_proxy_layer(proxy_helper, axis_basis, exponents)
    ...
    return proxy_helper(axis_basis, bundle)
end

function _materialize_cpb_mixed_gto_nuclear_by_center_block(
    parent::CPGB.CartesianParentGaussletBasis3D,
    cpb::CPB.CoordinateProductBox,
    supplement,
    expansion,
    ...
    internal_supplement = _cpb_gto_internal_supplement(supplement)
    proxy_layers = _cpb_mixed_gto_nuclear_proxy_layers(parent, expansion)
    factor_helper = getproperty(
        parentmodule(@__MODULE__),
        :_qwrg_atomic_axis_factor_cross_data,
    )
    intervals = _axis_named_tuple(CPB.intervals(cpb))
    local_shape = CPB.shape(cpb)
    dense_block = zeros(Float64, CPB.support_count(cpb), length(internal_supplement.orbitals))
    for (orbital_index, orbital) in pairs(internal_supplement.orbitals)
        axis_factors = (;
            x = factor_helper(proxy_layers.x, orbital, :x, expansion, center_summary.x),
            y = factor_helper(proxy_layers.y, orbital, :y, expansion, center_summary.y),
            z = factor_helper(proxy_layers.z, orbital, :z, expansion, center_summary.z),
        )
        for (local_ix, ix) in enumerate(intervals.x),
            ...
            value = 0.0
            # Match the existing QW/GTO nuclear_ga_by_center convention: this
            # is the uncharged per-center attraction contribution. The nuclear
            # charge is applied later when constructing a summed one-body block.
            @inbounds for term in eachindex(expansion.coefficients),
                    primitive in eachindex(orbital.coefficients)
                value -=
                    Float64(expansion.coefficients[term]) *
                    Float64(orbital.coefficients[primitive]) *
                    axis_factors.x[term][ix, primitive] *
                    axis_factors.y[term][iy, primitive] *
                    axis_factors.z[term][iz, primitive]
            end
            dense_block[row, orbital_index] = value
        end
    end
    return dense_block
end

function _materialize_cpb_gto_nuclear_by_center_block(
    supplement,
    expansion,
    center_summary,
)
    ...
    weighted_hadamard = getproperty(
        parentmodule(@__MODULE__),
        :_qwrg_atomic_weighted_hadamard,
    )
    orbital_count = length(internal_supplement.orbitals)
    dense_block = zeros(Float64, orbital_count, orbital_count)
    for (left_index, left) in pairs(internal_supplement.orbitals),
            (right_index, right) in pairs(internal_supplement.orbitals)
        axis_factors = (;
            x = factor_helper(left, right, :x, expansion, center_summary.x),
            y = factor_helper(left, right, :y, expansion, center_summary.y),
            z = factor_helper(left, right, :z, expansion, center_summary.z),
        )
        value = 0.0
        @inbounds for term in eachindex(expansion.coefficients)
            value -= Float64(expansion.coefficients[term]) * weighted_hadamard(
                left.coefficients,
                axis_factors.x[term],
                axis_factors.y[term],
                axis_factors.z[term],
        ...
        dense_block[left_index, right_index] = value
    end
    return Matrix{Float64}(0.5 .* (dense_block .+ transpose(dense_block)))
end
...
function _materialize_cpb_mixed_gto_kinetic_block(
    parent::CPGB.CartesianParentGaussletBasis3D,
    cpb::CPB.CoordinateProductBox,
    orbital,
)
    axis_tables = _cpb_mixed_gto_kinetic_axis_tables(parent, orbital)
    return _materialize_cpb_mixed_gto_dense_from_axis_table_terms(
        cpb,
        orbital,
        axis_tables,
    )
...
function _materialize_cpb_mixed_gto_dense_from_axis_table_terms(
    cpb::CPB.CoordinateProductBox,
    orbital,
    axis_table_terms,
)
    ...
    for (local_ix, ix) in enumerate(intervals.x),
        ...
        for primitive in eachindex(orbital.coefficients)
            ...
            for axis_tables in axis_table_terms
                value +=
                    coefficient *
...
function _cpb_mixed_gto_kinetic_axis_tables(
    parent::CPGB.CartesianParentGaussletBasis3D,
    orbital,
)
    axis_representations = _cpb_mixed_gto_parent_axis_representations(parent)
    overlap_tables = (;
        ...
        z = _cpb_mixed_gto_axis_overlap_table(axis_representations.z, orbital, :z),
    )
    kinetic_tables = (;
        x = _cpb_mixed_gto_axis_kinetic_table(axis_representations.x, orbital, :x),
        y = _cpb_mixed_gto_axis_kinetic_table(axis_representations.y, orbital, :y),
        z = _cpb_mixed_gto_axis_kinetic_table(axis_representations.z, orbital, :z),
    )
    return (
        (x = kinetic_tables.x, y = overlap_tables.y, z = overlap_tables.z),
        (x = overlap_tables.x, y = kinetic_tables.y, z = overlap_tables.z),
        (x = overlap_tables.x, y = overlap_tables.y, z = kinetic_tables.z),
    )
end
...
function _is_cartesian_gaussian_shell_supplement(supplement)
    return hasproperty(supplement, :supplement_kind) &&
           hasproperty(supplement, :orbitals) &&
           hasproperty(supplement, :metadata)
end
...
function _cpb_mixed_gto_axis_kinetic_table(axis_representation, orbital, axis::Symbol)
    basis_primitives = collect(primitives(primitive_set(axis_representation)))
    gausslet_module = parentmodule(@__MODULE__)
    gaussian_type = getproperty(gausslet_module, :Gaussian)
    all(primitive -> primitive isa gaussian_type, basis_primitives) || throw(
        ArgumentError(
            "mixed GTO kinetic pilot requires Gaussian 1D primitives on the Cartesian axis representation",
        ),
    )
    center_value =
        axis === :x ? orbital.center[1] :
    ...
    power =
        ...
        throw(ArgumentError("axis must be :x, :y, or :z"))
    shell_prefactor = getproperty(gausslet_module, :_qwrg_atomic_shell_prefactor)
    kinetic_integral = getproperty(gausslet_module, :_qwrg_atomic_kinetic_integral)
    gaussian_exponent = getproperty(gausslet_module, :_qwrg_gaussian_exponent)
    primitive_cross = zeros(Float64, length(basis_primitives), length(orbital.exponents))
    for (row, primitive) in pairs(basis_primitives)
        alpha_basis = gaussian_exponent(primitive)
        for column in eachindex(orbital.exponents)
            exponent = Float64(orbital.exponents[column])
            prefactor = shell_prefactor(exponent, power)
            primitive_cross[row, column] = kinetic_integral(
                alpha_basis,
                primitive.center_value,
                0,
                1.0,
...
function _cpb_mixed_gto_kinetic_block_summary(
    status::Symbol,
    blocker,
    parent::CPGB.CartesianParentGaussletBasis3D,
    cpb::CPB.CoordinateProductBox,
    ...
    available = status === :materialized_cpb_mixed_gto_kinetic_local_block
    local_shape = _axis_named_tuple(CPB.shape(cpb))
    primitive_count =
        hasproperty(orbital, :exponents) ? length(orbital.exponents) : :unavailable
    return (;
...
function _cpb_mixed_gto_supplement_block_summary(
    status::Symbol,
    blocker,
    parent::CPGB.CartesianParentGaussletBasis3D,
    cpb::CPB.CoordinateProductBox,
    ...
    available =
        status in (
            :materialized_cpb_mixed_gto_supplement_local_overlap_block,
            :materialized_cpb_mixed_gto_supplement_coordinate_moment_local_block,
            :materialized_cpb_mixed_gto_supplement_kinetic_local_block,
        )
    local_shape = _axis_named_tuple(CPB.shape(cpb))
    orbital_count = length(supplement.orbitals)
    blocked_orbital_summaries = Tuple(
        orbital_summary for orbital_summary in orbital_summaries if
        orbital_summary.dense_block_available != true
    )
    return (;
        object_kind = :cartesian_cpb_mixed_gto_supplement_local_block_summary,
        status,
        blocker,
        _cpb_mixed_gto_supplement_summary_convention_fields(orbital_count)...,
        term,
        source_kind,
        active_axis,
        coordinate_moment,
        ...
        source_metadata_summary = supplement.metadata,
        orbital_count,
        orbital_labels = Tuple(orbital.label for orbital in supplement.orbitals),
        orbital_angular_powers =
            Tuple(orbital.angular_powers for orbital in supplement.orbitals),
        orbital_centers = Tuple(orbital.center for orbital in supplement.orbitals),
        orbital_summaries,
        blocked_orbital_labels =
            Tuple(summary.orbital_label for summary in blocked_orbital_summaries),
        blocked_orbital_blockers =
            Tuple(summary.blocker for summary in blocked_orbital_summaries),
        formula_source = :GaussianAnalyticIntegrals_polynomial_gaussian,
        axis_kernel_source =
            term == :mixed_gto_overlap ?
        ...
        dense_block_eltype = available ? eltype(dense_block) : :unavailable,
        local_ordering = _LOCAL_ORDERING,
        representation = :dense_local_cpb_product_space_by_gto_supplement,
        galerkin_operator = true,
        provider_level_local_matrix_materialized = available,
        realization_status = :unrealized,
        route_global_status = :unassigned,
...
function _cpb_mixed_gto_nuclear_by_center_block_summary(
    status::Symbol,
    blocker,
    parent::CPGB.CartesianParentGaussletBasis3D,
    cpb::CPB.CoordinateProductBox,
    ...
    available = status == :materialized_cpb_mixed_gto_nuclear_by_center_block
    orbital_count =
        _is_cartesian_gaussian_shell_supplement(supplement) ?
        length(supplement.orbitals) :
        :unavailable
    ...
    return (;
        object_kind = :cartesian_cpb_mixed_gto_nuclear_by_center_block_summary,
        status,
        blocker,
        term = :electron_nuclear_by_center,
        source_kind = :mixed_gausslet_gto_supplement_nuclear_by_center,
        source_oracle_helper = :nuclear_ga_by_center,
        formula_source = :existing_qw_gto_nuclear_gaussian_factor_tables,
        factor_source_path = :_qwrg_atomic_axis_factor_cross_data,
        supplement_representation_kind =
            :cartesian_gaussian_shell_supplement_representation,
        supplement_kind =
            _is_cartesian_gaussian_shell_supplement(supplement) ?
        ...
        center_coordinates = center_summary.location,
        nuclear_charge = center_summary.charge,
        nuclear_charge_recorded = center_summary.charge != :unavailable,
        nuclear_charge_applied = false,
        oracle_convention = :qw_nuclear_by_center_uncharged,
        charge_application_stage = :hamiltonian_or_center_summation,
        by_center = true,
        ...
        nuclear_by_center_block_materialized = available,
        charged_nuclear_operator_materialized = false,
        provider_level_local_matrix_materialized = available,
        provider_level_pilot = true,
        mixed_gto_pilot = true,
        gto_supplement_self_block = false,
        realization_status = :unrealized,
        route_global_status = :unassigned,
        route_driver_wiring = false,
        route_global_matrix_materialized = false,
...
function _cpb_gto_nuclear_by_center_block_summary(
    status::Symbol,
    blocker,
    supplement,
    expansion,
    ...
    available = status == :materialized_cpb_gto_nuclear_by_center_block
    orbital_count =
        _is_cartesian_gaussian_shell_supplement(supplement) ?
        length(supplement.orbitals) :
        :unavailable
    ...
    coefficient_count = coefficients isa AbstractVector ? length(coefficients) : 0
    return (;
        object_kind = :cartesian_cpb_gto_nuclear_by_center_block_summary,
        status,
        blocker,
        term = :electron_nuclear_by_center,
        source_kind = :gto_supplement_nuclear_by_center,
        source_oracle_helper = :nuclear_aa_by_center,
        formula_source = :existing_qw_gto_nuclear_gaussian_factor_tables,
        factor_source_path = :_qwrg_atomic_axis_factor_aa_data,
        supplement_representation_kind =
            :cartesian_gaussian_shell_supplement_representation,
        supplement_kind =
            _is_cartesian_gaussian_shell_supplement(supplement) ?
...
function _cpb_mixed_gto_supplement_summary_convention_fields(orbital_count::Int)
    return (;
        contraction_convention =
            :orbital_coefficients_contract_primitive_axis_tables,
        center_shift_convention = :explicit_axis_center_coordinates,
        ...
        per_orbital_right_basis_kind = :single_cartesian_gaussian_shell_orbital,
        provider_level_pilot = true,
        mixed_gto_pilot = true,
        parent_one_body_factor_packet_consumed = false,
        mixed_gto_axis_integral_source =
