...
It must not force a persistent shell object after splitting, fake compactness
by dropping rows or relabeling full-support units, change the driver, artifact
schema/provenance, PQS behavior, Hamiltonian assembly, raw blocks, RG/MWG/IDA,
old WL H1/H1+J materialization, diagnostics/status/report payloads, committed
tests/fixtures, or Cr2 workflow. The same public `ns` is the fair starting
input for PQS/WL comparison after this correction, but it is not a promise of
