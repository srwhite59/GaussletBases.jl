...
and `execution_whitelist.md` are deterministic checked views; they grant no
...
An ID must be authorized by `authority.toml` and listed in the generated
`execution_whitelist.md` before it permits source-bearing work. `current.md`
need not enumerate every active ID; silence there is neutral. If the machine
authority, generated views, an explicit current-status statement, or canonical
contract disagree, make no producer edit and request a docs-only
...
Before source work, inspect the exact assigned `authority.toml` record and
confirm its ID in `execution_whitelist.md`. The record owns the grant, paths,
surfaces, and scope; the whitelist grants nothing independently. Missing or
...
The execution IDs are lexically sorted. Their order carries no lifecycle,
