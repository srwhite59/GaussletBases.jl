# Runtime role: focused contract / boundary.
#
# This is not the preferred routine per-pass smoke. It preserves broad
# accessor and local one-body block collection coverage that was split out of
# the small mixed one-body smoke test. It still uses synthetic metadata only
# and avoids the real White-Lindsey adapter fixture.
...
@testset "CartesianPairBlockMaterialization mixed one-body accessor/collection contract" begin
