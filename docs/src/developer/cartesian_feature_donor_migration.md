...
|---|---|---|---|---|---|---|---|
| Residual-GTO / MWG supplement materialization | `ordinary_qw_residuals.jl`; `ordinary_qw_raw_blocks.jl`; `ordinary_qw_operator_assembly.jl`; `cartesian_gto_probes.jl`; residual-GTO and mixed gausslet/GTO block helpers | mixed public/internal | Current PQS GTO path reaches supplement preflight only; no mixed/GTO provider blocks, residual MWG representation, or supplemented values are produced by the thin route. | Existing `*_gto` ladder cases first; later add a materialized supplement case for WL and PQS when provider blocks exist. | CPB provider-block supplement stage plus compact supplement artifact contract. | Delete or shrink donor pieces after thin route builds mixed gausslet/GTO blocks, GTO/GTO blocks, residual representation, and supplemented values, or after the feature is intentionally abandoned. | P1 |
| Ham/JLD2 artifact contract and basis transfer/roundtrip | `cartesian_basis_representation.jl`; `cartesian_cross_overlap.jl`; `cartesian_representation_transfer.jl`; `cartesian_carried_spaces.jl`; current JLD2 writer patterns | public/mixed | Thin route can run and print compact facts, but the final consumer artifact and transfer/roundtrip contract is not yet the main authority. | `he_wl_artifact_roundtrip`, then `h2_pqs_artifact_roundtrip` once H2 payloads are stable. | Compact driver artifact writer for basis, H1/H1-J/RHF/supplement/provenance plus transfer smoke. | Donor line can be deleted or retained narrowly once thin artifacts can be loaded and used by downstream consumers without old representation wrappers. | P1 |
...
1. P1 residual-GTO / MWG supplement materialization.
2. P1 Ham/JLD2 artifact contract and basis transfer/roundtrip.
