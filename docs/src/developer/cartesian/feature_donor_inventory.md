...
|---|---|---|---|---|---|---|---|
| Residual-GTO / MWG supplement materialization | `ordinary_qw_residuals.jl`; `ordinary_qw_raw_blocks.jl`; `ordinary_qw_operator_assembly.jl`; `cartesian_gto_probes.jl`; residual-GTO and mixed gausslet/GTO block helpers | mixed public/internal | Thin route can materialize a PQS Ham/Basis plus residual-GTO sidecar artifact for the H2 independent PQS route, including residual-GTO one-body and density-provider blocks with `provider_block_mode = :one_body_and_density_provider`. It still lacks residual MWG supplemented production values and a full provider-block Hamiltonian. | `h2_pqs_q5_independent_source_box_r4_gto_materialized` in the `pqs_diatomic` line ladder; future cases should cover supplemented values and external consumer use. | CPB provider-block supplement stage plus compact supplement artifact contract. | Delete or shrink donor pieces after thin route builds mixed gausslet/GTO blocks, GTO/GTO blocks, residual representation, density/pair/H1-J provider blocks, and supplemented values, or after the feature is intentionally abandoned. | P1 |
