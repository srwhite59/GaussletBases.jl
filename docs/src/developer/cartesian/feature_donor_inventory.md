...
|---|---|---|---|---|---|---|---|
| Residual-GTO / MWG supplement materialization | `ordinary_qw_residuals.jl`; `ordinary_qw_raw_blocks.jl`; `ordinary_qw_operator_assembly.jl`; `cartesian_gto_probes.jl`; residual-GTO and mixed gausslet/GTO block helpers | mixed public/internal | H2 independent PQS can materialize a private residual-GTO H/V/T handoff with `H` in `(:final_pqs, :residual_gto)`, `V` in `(:pre_final_pqs, :residual_gto)`, an explicit orbital-to-density transform, and `provider_block_mode = :one_body_and_density_provider`. This remains H2-fixture/private producer work, not a general public Cr2 producer. | `h2_pqs_q5_independent_source_box_r4_gto_materialized` in the `pqs_diatomic` line ladder; future cases should cover external consumer use. | Public/neutral Ham contract plus compact supplement artifact contract. | Delete or shrink donor pieces after a public producer absorbs mixed gausslet/GTO blocks, GTO/GTO blocks, residual representation, MWG interaction kernels, and consumer-validated H/V/T output, or after the feature is intentionally abandoned. | P1 |
| Ham/JLD2 artifact contract and basis transfer/roundtrip | `cartesian_basis_representation.jl`; `cartesian_cross_overlap.jl`; `cartesian_representation_transfer.jl`; `cartesian_carried_spaces.jl`; current JLD2 writer patterns | public/mixed | H2 residual-GTO sidecar artifacts write/read back a minimal H/V/T handoff and no-solver consumer invariant. This does not yet provide a public reader API, Cr2-ready producer, full supplemented values, or provider-block Hamiltonian production beyond the current private H2 artifact. | `h2_pqs_q5_independent_source_box_r4_gto_materialized`; later add downstream H2 PQS consumer roundtrip cases. | Compact public artifact writer/reader for basis, H/V/T/provenance plus transfer smoke. | Donor line can be deleted or retained narrowly once thin artifacts can be loaded and used by downstream consumers without old representation wrappers. | P1 |
...
1. P1 residual-GTO / MWG supplement materialization: private H2 residual-GTO
   H/V/T handoff is materialized with one-body and density-provider blocks;
   remaining work is a public/neutral Ham contract, arbitrary electron/spin and
...
2. P1 Ham/JLD2 artifact contract and basis transfer/roundtrip: H2 sidecar
   artifacts write/read back the minimal H/V/T handoff and no-solver consumer
   invariant; remaining work is public reader/writer design and downstream
   consumer coverage, not private H1-J/RHF solver diagnostics.
