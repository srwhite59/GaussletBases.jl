...
| `pqs_diatomic` | `test/driver_inputs/h2_pqs_q5_independent_source_box_r4.jl`; `test/driver_inputs/h2_pqs_q5_independent_source_box_r4_supplement_preflight.jl`; `test/driver_inputs/h2_pqs_q5_independent_source_box_r4_supplement_materialized.jl` | Protect the independent H2 PQS source-box line, supplement preflight, and H2 PQS residual-GTO materialization. | Independent route remains fake-free; final dimension is 471 for the materialized H2 PQS Ham case; overlap error is finite/small; H1 is finite/symmetric with finite lowest energy; H1-J self-Coulomb is finite/positive as a diagnostic, not a producer contract; the Ham artifact is the public Cartesian IDA Hamiltonian artifact with `K`, `{U_A}`, `Vee`, charges, `ncenter x 3` positions, and spin counts. | Delete or fold into main matrix once independent PQS and residual-GTO materialization are ordinary route functionality with compact consumer checks. |
