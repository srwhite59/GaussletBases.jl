...
| `pqs_diatomic` | `test/driver_inputs/h2_pqs_q5_independent_source_box_r4.jl`; `test/driver_inputs/h2_pqs_q5_independent_source_box_r4_supplement_preflight.jl`; `test/driver_inputs/h2_pqs_q5_independent_source_box_r4_supplement_materialized.jl` | Protect the independent H2 PQS source-box line, supplement preflight, and H2 PQS residual-GTO materialization. | Independent route remains fake-free; final dimension is 471 for the materialized H2 PQS sidecar case; overlap error is finite/small; H1 is finite/symmetric with finite lowest energy; H1-J self-Coulomb is finite/positive as a diagnostic, not a producer contract; the basis/residual sidecar is written and reloadable; the Ham artifact is the public Cartesian IDA Hamiltonian artifact with `K`, `{U_A}`, `Vee`, charges, `ncenter x 3` positions, and spin counts; the private basis/residual sidecar still records `provider_block_mode = :one_body_and_density_provider`. | Delete or fold into main matrix once independent PQS and residual-GTO sidecar materialization are ordinary route functionality with compact consumer checks. |
...
2. P1 Ham/JLD2 artifact contract and basis transfer/roundtrip: the Ham side has
   a public Cartesian IDA writer/reader. The basis/residual sidecar remains
   private; private H1-J/RHF solver diagnostics are not part of the producer
   contract.
