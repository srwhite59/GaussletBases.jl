...
was deleted without replacement in Pass 467 under
...
| contracted-parent/metrics, multilayer, and complete-core-shell oracle cluster | Deleted abandoned alternative planning/oracle route with no current numerical consumer. | None. Current parent, terminal, PQS/WL, pair, raw-block, PRF, represented-Hartree, and IDA owners remain independent. | Exact caller/deleted-symbol scans, package load, unchanged `core`/`ida`/`cartesian` groups, and public endpoint due diligence passed. | retired | Reintroduction requires new authority; no compatibility surface remains. | Pass 467 deleted exactly 10,590 tracked lines with no alias, shim, replacement helper, moved kernel, or probe adapter. |
