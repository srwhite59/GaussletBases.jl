...
`CartesianCPBBlockProviders.jl` pilot was deleted in Pass 465 under the
...
They do not authorize deletion or migration of pair-materialization or other
surviving source. The separately audited contracted-parent/multilayer cluster
is now approved for exact no-replacement deletion under
[HP-RETIRE-CONTRACTED-PARENT-FN-01](designs/cartesian_hamiltonian_producer/contracted_parent_multilayer_retirement.md);
older contracted-parent oracle and migration statements below are historical.

| Old surface / file | Current role | Replacement module/stage | Equivalence test | Status | Deletion condition | Notes |
| --- | --- | --- | --- | --- | --- | --- |
| retired `src/CartesianCPBBlockProviders.jl` | Deleted orphaned experimental local-block provider. | None. Current CPB geometry, parent factors, raw blocks, terminal operators, and represented fields retain their existing owners. | Package load and unchanged `core`, `ida`, and `cartesian` groups passed after exact file/include deletion. | retired | Reintroduction requires new authority; no compatibility surface remains. | Pass 465 deleted exactly 6,446 source lines. External July Cr/Cr2 probes may remain pinned to an old commit. |
| contracted-parent/metrics, multilayer, and complete-core-shell oracle cluster | Abandoned alternative planning/oracle route with no current numerical consumer. | None. Current parent, terminal, PQS/WL, pair, raw-block, PRF, represented-Hartree, and IDA owners remain independent. | Exact caller/deleted-symbol scans, package load, unchanged `core`/`ida`/`cartesian` groups, and public endpoint due diligence. | approved-for-retirement | Delete the exact 10,590 tracked lines under `HP-RETIRE-CONTRACTED-PARENT-*`; stop if a live caller or side effect appears. | No alias, shim, replacement helper, moved kernel, or probe adapter is authorized. |
...
| Mixed one-body block-set view/accessors | Retired private diagnostic layer over mixed one-body block-set consumption results. | Local one-body block collection plus placement-plan path. | Focused `rg` should show no surviving live source/test/tool/docs callers of `_one_body_pair_block_set_view`, `_one_body_pair_block_term_status`, `_one_body_pair_block_pair_status`, `_one_body_pair_block_lookup`, and support helpers after deletion. Package load and active CPBM one-body placement/oracle smokes should still pass. | approved-for-retirement | Delete the diagnostic view/accessor helpers once local collection construction and placement entry shape remain unchanged. | This is not permission to delete block-set consumption, batch summaries needed by local collection construction, local collection entries, placement planning, numerical kernels, or public materialization entry points. |
| CPBM local one-body materialization/nonclaim field cloud | Private false-claim/status vocabulary left in `one_body_dispatch.jl` and `one_body_block_collection.jl`. | Local collection entries plus placement-plan inputs should carry only facts consumed by the live local/global one-body path. | Focused `rg` should show deleted false-only fields are gone from the two approved files or remain only where live consumers require them. Package load must pass; run a focused CPBM one-body placement/oracle smoke if placement-facing entry fields change. | approved-for-narrow-cleanup | Remove false-only route/global/nonclaim fields from the two CPBM one-body files when they are unused private disclaimers. | Approved files are only `src/cartesian_pair_block_materialization/one_body_dispatch.jl` and `src/cartesian_pair_block_materialization/one_body_block_collection.jl`. Do not change adjacent providers/final-basis surfaces, numerical blocks, public materialization entry points, placement semantics, IDA/MWG, artifacts, route wiring, object-kind assertions, or repeated summary/status architecture in this pass. |
...
test. The active H and H2+ acceptance paths now materialize decomposed
route-global overlap, kinetic, separated electron-nuclear by-center matrices,
and a charge-applied one-electron Hamiltonian from a 27-unit, 378-pair
decomposed inventory. The direct-core retained unit covers columns `1:125`,
boundary units cover `126:223`, and the retained overlap metric is full rank for
...
- A narrow decomposed WL one-electron Hamiltonian assembly helper now consumes
  ...
  The current H and H2+ audits reach this assembled Hamiltonian and solve
  through the decomposed WL path. The direct-core retained operator inventory
  supplies columns `1:125`, the shell boundary inventory supplies `126:223`,
  and the overlap metric is positive definite with rank estimate 223.
