...
- [Parent-backed injected composition](parent_backed_injected_composition.md)
  for implemented fixed-span angular-style injection, numerical-complete
  external residuals, separated IDA/Gaussian/MWG blocks, and screened-Hartree
  delegation after the native Hamiltonian is fixed
...
- [PQS v0.2 public surface](pqs_public_surface.md) for the implemented bounded
  ...
  its isolated Julia `1.12.6` candidate replay is accepted, while the bounded
  dependency-compatibility declaration remains pending before release review
...
- [Contracted-parent and multilayer retirement](contracted_parent_multilayer_retirement.md)
  records completed deletion of the obsolete alternative representation,
...
- [Cartesian pair planning and materialization retirement](cartesian_pair_planning_materialization_retirement.md)
  records completed removal of the unused pair ladder while retaining the
  mapped-COMX axis-transform owner at its current path
