...
`HP-COMP-OUTERMM-*` adds the matching lowering guardrail for thickness-1
outer-mismatch slabs: PQS and White-Lindsey use the same compact
`ns x ns x 1` boundary-slab lowering and neither may retain those slabs as
full identity rows.
