...
The approximate Hartree reference operator must be the direct/Hartree part of
the same IDA/MWG Hamiltonian convention that the solver uses for the same
represented `P0`. For the current two-index Cartesian IDA convention this is:

```text
q = diag(P_alpha) + diag(P_beta)
E_app_direct = 1/2 * q' * V * q
F_app_direct = Diagonal(V * q)
```

Do not subtract the full approximate interaction Fock when the exact side is
Hartree-only. The same-spin exchange-like term in the current approximate
interaction remains part of the full corrected interaction model, but it is not
part of the Hartree reference subtraction. Do not invent a row-vector shortcut,
`q0` shortcut, or center-metadata substitution unless it is exactly the solver
convention.
...
term, from an exact Hartree operator. That result remains useful evidence for
source plumbing, but it must not be used as evidence that the Hartree
reference-density correction is physically stable.
...
a Hartree-correction physics/stability result. A valid rerun must use
...
new source. After `HP-RHO0-JANCHOR-*`, it consumes the direct-Hartree anchor
pieces:
...
the direct-Hartree correction defined by `HP-RHO0-JANCHOR-*`.
...
- H, Be, and Be2 only;
- consume current source-backed exact side, direct-Hartree approximate side,
  and anchor helper;
...
Status: approved narrow source authority for direct-Hartree reference-anchor
replacement.

Purpose: replace the invalid full-interaction subtraction in
`HP-RHO0-ANCHOR-*` with a Hartree-only approximate-side object. The exact side
is still `F_exact_Hartree[P0]`; only the approximate side changes to the
direct/Hartree part of the Cartesian IDA interaction:
...
- `src/cartesian_ida_hamiltonian.jl` for private/internal direct-only
  approximate Hartree energy/Fock helper(s);
...
- add internal direct-only Cartesian IDA helpers for `E_app_direct` and
  `F_app_direct` using the stored two-index IDA/MWG matrix `V`;
- keep the paired energy/Fock finite-difference discipline from
  `HP-RHO0-FAPP-*`;
- form one spin-independent `Delta_J0` matrix and `C0_J`;
- verify the direct-Hartree anchor:

```text
E_app_direct[P0] + Tr((P0_alpha + P0_beta) * Delta_J0) + C0_J
    = E_exact_Hartree[P0]

F_app_direct[P0] + Delta_J0
    = F_exact_Hartree[P0]
```
...
```text
dE_corr_int/dP_sigma = F_exact_Hartree[P0] - K_app_sigma[P0]
```

  where `K_app_sigma[P0]` denotes the current approximate same-spin
  exchange-like contribution already present in `E_app_full_int`;
- report `Delta_J0` spectra, diagonal/range summaries, occupied/reference
...
- rerun the H/Be/Be2 corrected-Hamiltonian audit with `Delta_J0` and `C0_J`
  after the source helper validates.
...
- row action `(J*w)/w`, `diag(J)`, `q0`, center metadata, direct `C' V C`, or
  IDA proxy shortcuts;
- residual/MWG defaults, residual selection changes, basis-fate policy, or
  broad rejected directions as MWG residuals;
- source files outside the approved surface;
- committed fixtures.
...
- `git diff --check`;
- package load;
- compact direct-only finite-difference validation:

```text
d E_app_direct[P] / dP_alpha = F_app_direct[P]
d E_app_direct[P] / dP_beta  = F_app_direct[P]
```
