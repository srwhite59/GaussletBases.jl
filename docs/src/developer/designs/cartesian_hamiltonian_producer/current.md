...
| `HP-RETIRE-PQS-STAGED-DESCRIPTOR-*` | Inert projected-q-shell staged descriptor approved for bounded retirement | Delete only the private descriptor/prototype closure and descriptor-only propagation/status fields in the two authorized source files; preserve exact active projected-q-shell coefficients, packets, fingerprints, ordinary/mapped behavior, and public endpoints, with no replacement or test edit |
