...
| `HP-PUBLIC-EXPERT-GEOMETRY-DOC-*` | Expert bond-aligned geometry inspection documentation approved; no export change authorized | Document the central inspection payload with its existing geometry family, require `24 -> 23`, and preserve two non-executable next-minor namespace reservations; no source deletion, performance, v0.2.x, version, changelog, release, or automatic v0.3 work |
