...
| `HP-PUBLIC-PARTITION-LEAF-DOC-*` | Partition-hierarchy and leaf-local accessor documentation approved; implementation pending | Document only the twelve existing accessors and listed public context in the existing bases reference; preserve copy versus stored/read-only ownership and require exact `52 -> 40` undocumented-export reduction without behavior or global docs-policy changes |
