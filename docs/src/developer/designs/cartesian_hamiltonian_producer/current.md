...
| `HP-PUBLIC-PAPER-CI-*` | Three-gate implementation landed; final acceptance pending one timeout-only rerun | Preserve the separate Julia `1.12` PQS and screening groups and unchanged Julia `1.10` supported-floor gate; change only the inherited `15`-minute timeout to `30`, require all three remote rows plus Docs to pass, and leave nested research suites unwired |
