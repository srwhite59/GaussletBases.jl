...
| `HP-FN-00` / `HP-MCOMX-TERM-FN-01` | Direct support-local PQS shell-seed assembly implemented; maintenance | Preserve byte-identical ordinary/mapped atomic/diatomic coefficients and all public endpoints without restoring global-full helpers; cold compilation, terminal nuclear/product buffers, Gram policy, and compatibility cleanup remain separate |
| `HP-FN-03` / `HP-DRV-STAGE-FN-01` | Call-local terminal product/nuclear buffer pre-sizing and reuse approved pending implementation | Reuse only the existing lexical buffers across three kinetic terms and physical centers; preserve every matrix bit, the exact 135-term loop, all endpoint facts, and reduce combined allocation by at least 3.0 GiB without persistent state |
...
| `HP-PQS-PUBLIC-MATCHED/SCREEN-*` | Both public surfaces implemented; matched-H2+ private parent-apply workspace reuse accepted, screening remains maintenance | Preserve disjoint per-comparison scratch, exact action parity, the frozen `12789/1285/1285` result and all diagnostics; leave screening, release, registration, and citation decisions separate |
...
| `HP-PUBLIC-PAPER-CI-*` | Three-gate paper-aligned CI and corrected path-aware routing implemented; maintenance | Preserve all three names/rows and PR behavior; expand named no-op gates for proven docs-only pushes, instantiate the docs environment in the lightweight lane, retain fail-closed full routing otherwise, and leave performance/H2+ rewiring separate |
