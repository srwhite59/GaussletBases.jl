...
| --- | --- | --- |
| `HP-AUTHORITY-EXECUTION-WHITELIST-*` | Whole-file execution-whitelist relocation implemented/completed; maintenance | Preserve `authority.toml` as sole authority, both generated checked views, stable fail-closed `AGENTS.md` instructions, exact executable-ID intersection semantics, and missing/stale/modified/nongenerated-view failures; grant no schema, source/API, workflow, or reverse-parser change |
...
| `HP-ANGULAR-TEST-CONTROL-*` | Optional-consumer control-flow repair implemented/completed; maintenance | Preserve the local visible skip for a missing HFDMRG checkout, propagated failures from an existing checkout, complete explicit `angular` selection, and exclusion from `fast`; no numerical, CI, API, or broader angular change |
...
| `HP-SOURCE-LAYOUT-RADIAL-ATOMIC-*` | Exact 14-file radial/atomic path-only relocation implemented; validation completed; maintenance | Preserve the 14 byte-identical files under `src/radial/` and `src/atomic/`, their in-place root include order, reconciled current pointers, frozen hashes, and complete-renaming evidence; grant no file-body, test, module, API, numerical, workflow, release, or later-layout change |
| `HP-SOURCE-LAYOUT-ANGULAR-*` | Exact five-file angular path-only relocation implemented; validation completed; maintenance | Preserve the five byte-identical owners under `src/angular/`, their include positions, reconciled current pointers, and unchanged `_lanczos_ground_state_apply`; grant no file-body, test, workflow, API, numerical, helper-relocation, release, or later-layout change |
| `HP-SOURCE-LAYOUT-FOUNDATION-*` | Exact 16-file foundation path-only relocation implemented; validation completed; maintenance | Preserve the 13 byte-identical files and three four-comment-only files under `src/foundation/`, exact include order and path pointers, accepted tree `20faffb58`, and unchanged package behavior; grant no executable, test, workflow, numerical, Step 4, or later-layout change |
| `HP-SOURCE-LAYOUT-ORDINARY-*` | Exact 15-file Ordinary path-only relocation implemented/completed; maintenance | Preserve commit `96802b00c`, the 15 byte-identical owners under `src/ordinary/`, their exact include positions, classified current pointers, frozen hashes, and R100 identities; grant no body, test, workflow, API, numerical, Cartesian/PQS, or later-layout change |
| `HP-SOURCE-LAYOUT-CARTESIAN-*` | Exact 76-file Cartesian/PQS path-only relocation implemented/completed; maintenance | Preserve commit `64d8d65b6`, the 76 R100 owners under `src/cartesian/`, 39 in-place root includes, normalized include identity, and classified current pointers; grant no body, test, workflow, API, numerical, follow-on, or release change |
...
| `HP-PUBLIC-PAPER-CI-*` | Three-gate CI and `docs_fast` source-push integrity owner implemented; maintenance | Preserve the shared mechanical public-surface owner, exact five-name reserved set, Julia 1.10 fallback and Julia 1.12 native parity, once-only docs/full-docs selection, unchanged `fast` alias, all numerical groups, names/rows, classifier, paper/tag lanes, and every nonmechanical docs check |
| `HP-CARTESIAN-INTERNAL-MAINTENANCE-CI-*` | Existing four-suite workflow remains implemented; fifth occupied-first process approved | Add only the unchanged 64-check Be/Ne occupied-first owner after atomic packet; preserve the `15`-minute Julia `1.12` read-only job, every existing command's relative order, cadence, triggers, permissions, and public CI; represented Hartree remains standalone and blocked |
