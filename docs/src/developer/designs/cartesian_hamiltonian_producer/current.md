...
| `HP-REP-MIXDENS-HARTREE-*` | Direct represented-density Hartree producer approved; Pass 459 preflight exposed the missing pair action | Implement a tiled contracted PGDG/Gaussian source-pair action for complete `GG`, `GA/AG`, and `AA` density sectors, then materialize the complete field; no dense pair matrix, IDA substitute, or fitted route |
