...
| `HP-FN-00` / `HP-MCOMX-TERM-FN-01` | Direct support-local PQS shell-seed replacement approved pending implementation; the live source still materializes global full coefficients before restriction | Delete both global-full private helpers, prove bitwise parity for ordinary/carried one-center and diatomic blocks, preserve every public endpoint, and reduce isolated seed allocation from about 1 GB toward the measured 14 MB without net source growth |
...
| `HP-PUBLIC-PAPER-CI-*` | Three-gate paper-aligned CI implemented; fail-closed path-aware routing approved pending implementation | Preserve all three names/rows and PR behavior; route only proven documentation-only main pushes to lightweight package/docs checks, use a separate annotated-tag identity/install lane, and leave performance/H2+ rewiring separate |
