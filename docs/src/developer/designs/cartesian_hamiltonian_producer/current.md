...
| `HP-PQS-PUBLIC-V020-RELEASE-*` | Exact conditional annotated-tag and final GitHub-release transaction approved; pending execution | Target only `adfcaba32`, require ordered tag/remote-install/Docs-Pages checks before publishing the exact `2,278`-byte final/latest release with zero assets, and preserve any partial tag or release without mutation |
