...
| `HP-FN-03` / `HP-DRV-STAGE-FN-01` | Call-local terminal product/nuclear buffer pre-sizing and reuse implemented; maintenance | Preserve the pre-sized lexical buffers across three kinetic terms and physical centers, every matrix bit, the exact 135-term loop, and all endpoint facts; no persistent state or arithmetic reorder |
