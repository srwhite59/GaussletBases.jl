...
| Parent-backed functions | PRF mechanics and exact descriptor-to-PRF source binding remain implemented internally; six unsupported PRF-specific root exports are approved for de-promotion without changing the numerical facility | [Parent residual functions](parent_residual_functions.md), [parent-backed injected composition](parent_backed_injected_composition.md) |
...
| `HP-PQS-PRF-CONSUMER-*` | API de-promotion approved | Delete only the six PRF root exports and qualify their nine existing nested-test uses; retain implementations, types, numerics, and the separately owned `cartesian_base_working_basis` export |
