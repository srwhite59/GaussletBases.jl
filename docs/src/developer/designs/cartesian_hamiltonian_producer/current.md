...
| `HP-ANGULAR-TEST-CONTROL-*` | Pass 559 control-flow repair remains closed; explicit HFDMRG test-path portability authorized | Replace only the personal path in `_local_hfdmrg_module()` with `GAUSSLETBASES_HFDMRG_SRC`; preserve the cache, visible absence skip, propagated configured-path failures, later tests, explicit `angular` selection, and exclusion from `fast` and public CI |
