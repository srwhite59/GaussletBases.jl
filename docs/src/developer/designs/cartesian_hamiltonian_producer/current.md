...
| `HP-PQS-PUBLIC-RC1-*` | `v0.2.0-rc1` candidate prepared at `41fa897ae`; clean validation completed; maintenance | Preserve the exact candidate version and concise reader changelog; tag and GitHub-prerelease operations are separately bounded, while registration, citation metadata, and final-release decisions remain unauthorized |
| `HP-PQS-PUBLIC-RC1-RELEASE-*` | One package-centered GitHub prerelease approved, pending publication | Publish only against the immutable RC1 tag with the exact separate PQS/screening narrative, prerelease and non-latest status, and no uploaded assets or tracked-file mutation |
| `HP-PQS-PUBLIC-RC1-TAG-*` | Immutable annotated tag and versioned documentation accepted; lifecycle closed | Preserve object `a4284f0bf`, frozen target `1546c18d3`, RC1/dev selector entries, and absent `/stable/`; release publication is owned only by the separate RC1 release record |
