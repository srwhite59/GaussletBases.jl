...
| `HP-SOURCE-LAYOUT-RADIAL-ATOMIC-*` | Exact 14-file radial/atomic path-only relocation implemented; validation completed; maintenance | Preserve the relocation targets, include positions, pointers, and frozen relocation evidence; the shared-Lanczos grant alone may simplify the atomic two-electron body while preserving public behavior |
| `HP-SOURCE-LAYOUT-ANGULAR-*` | Exact five-file angular path-only relocation implemented; validation completed; maintenance | Preserve the five byte-identical owners, include positions, and current pointers; the separately authorized shared-Lanczos transaction is the sole exception to its frozen helper placement |
| `HP-SOURCE-LAYOUT-FOUNDATION-*` | Exact 16-file foundation path-only relocation implemented; validation completed; maintenance | Preserve the accepted relocation and package behavior; the separately authorized shared-Lanczos transaction alone may add one foundation owner and one include without reordering existing entries |
| `HP-FOUNDATION-LANCZOS-*` | Shared private apply-based reference kernel and thin atomic wrapper approved for implementation | Move the exact angular helper body to one foundation owner, preserve all private callers and public atomic behavior bitwise, remove the duplicate algorithm, stay net-negative, and stop on parity, message, allocation, compilation, or budget failure |
...
| `HP-SOURCE-LAYOUT-CARTESIAN-*` | Exact 76-file Cartesian/PQS path-only relocation implemented/completed; maintenance | Preserve commit `64d8d65b6`, its 76 R100 owners, 39 moved includes, and pointers; its 88-entry hash remains relocation evidence while the Lanczos grant may insert one earlier foundation include |
