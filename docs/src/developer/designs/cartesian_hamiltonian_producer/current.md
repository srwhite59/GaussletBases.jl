...
| `HP-PUBLIC-CARTESIAN-FRONTDOOR-DOC-*` | Public facade documentation repair approved; implementation pending | Document the exact atom/diatomic schemas, supported public `q`/`ns` normalization, atom-only `d`, one bounded PQS snippet, and `Vee` field identity without another numerical docs run; main repairs `/dev/`, while immutable v0.2.0 `/stable/` remains unchanged |
