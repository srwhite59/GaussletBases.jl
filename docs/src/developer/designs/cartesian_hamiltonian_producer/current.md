...
| --- | --- | --- |
| `HP-ANGULAR-PUBLIC-CI-*` | Fixed-radial `[10,15,32]` public-owner extraction approved, not implemented | Move and condense the existing block into `angular_public`, append only that group to Julia 1.10 Supported floor, keep `all` coverage and exclude both angular groups from `fast`; full angular research CI, export documentation, and shell-key de-export remain separate |
...
| `HP-PUBLIC-PAPER-CI-*` | Supported-floor `radial,misc` extension implemented; bounded `angular_public` addition separately approved | Preserve the three names/rows and every other workflow behavior; permit only the pending Julia 1.10 append of `angular_public`, never bare `angular` |
