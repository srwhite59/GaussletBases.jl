...
| `HP-PQS-PUBLIC-COMPAT-*` | Exact six-bound root compatibility amendment approved, not implemented | Add only the declared `[compat]` lines, prove them by isolated Julia `1.12.6` resolution plus Julia `1.10` CI, and return for docs-only closeout; no version, tag, manifest, workflow, source-code, or test change |
