...
| `HP-PUBLIC-PAPER-CI-*` | Three-gate paper-aligned CI and fail-closed path-aware routing implemented; maintenance | Preserve all three names/rows and PR behavior; route only proven documentation-only main pushes to lightweight package/docs checks, use a separate annotated-tag identity/install lane, and leave performance/H2+ rewiring separate |
