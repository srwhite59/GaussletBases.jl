...
| `HP-SLICE-HCHAIN-*` | Producer implemented; one sixth expert query approved pending implementation | Add only `sliced_h1_bandwidth` as an O(1), zero-allocation structural query over existing state, with exact off-band-zero and singleton tests; preserve all operator physics and add no field, solver, or dense matrix |
