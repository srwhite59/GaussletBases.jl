...
| `HP-PUBLIC-EXPORT-INTEGRITY-*` | Three-name post-v0.2 export cleanup approved; implementation pending | Delete unused `TimedNestedFixedBlockBuild`; unexport but retain `OneCenterAtomicNestedLayerStructure` and the qualified QW operator alias; preserve `ShellLocalAngularProfileKey`, numerical behavior, dynamic integrity coverage, and immutable releases |
