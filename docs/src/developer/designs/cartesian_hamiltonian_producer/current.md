...
| `HP-MAPPED-ORDINARY-REP-*` | Exact two-function Foundation-to-Ordinary ownership move approved for implementation | Move the frozen 25-line block byte-identically after mapped proxy localization, leave the Cartesian caller unchanged, add only the bounded core representation checks, and stop on any parity, caller, or budget failure |
