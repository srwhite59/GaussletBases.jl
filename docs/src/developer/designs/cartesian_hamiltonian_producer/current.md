...
| `HP-PQS-PUBLIC-DOC-PARITY-*` | Bounded reader-document repair approved, not implemented | Add the eleven accepted v0.2 exports to the curated reference, remove personal paths from two rendered algorithm pages without deleting provenance, and mark example 28 internal/non-contractual; no API, example-code, citation, or release change |
