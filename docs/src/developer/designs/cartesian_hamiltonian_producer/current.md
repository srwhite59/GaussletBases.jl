...
| `HP-PUBLIC-SLICED-HCHAIN-DOC-*` | Three sliced-chain operator functions approved for documentation; implementation pending | Add only their docstrings, one compact six-export expert-reference section, and focused docs checks; require the exact undocumented-export reduction `9 -> 6` while preserving lazy/read-only views, compact storage, zero-allocation compatible-buffer rows, and all sliced-chain numerical behavior |
