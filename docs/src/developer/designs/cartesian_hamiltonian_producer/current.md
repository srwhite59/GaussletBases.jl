...
| `HP-PQS-PUBLIC-V020-RELEASE-*` | Immutable tag `722e8e875` is correct; final release remains blocked only on one bounded manual tag-verification recovery lane | Preserve the tag at `adfcaba32`; verify it through a fresh namespaced scratch ref after CI `33130411193` hit a local lightweight-tag collision, then recheck accepted Docs/Pages before publishing the exact `2,278`-byte final/latest release with zero assets |
...
| `HP-PUBLIC-PAPER-CI-*` | Three-gate paper-aligned CI and PQS single-execution repair implemented; maintenance | Preserve all three names/rows, PR behavior, path-aware routing, screening gate, workflow bytes, and one Example 41 comparison feeding all 18 release checks; do not restore the duplicate subprocess |
