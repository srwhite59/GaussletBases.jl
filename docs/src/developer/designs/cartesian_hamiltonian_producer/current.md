...
| `HP-PQS-PUBLIC-MATCHED/SCREEN-*` | Both public surfaces implemented; matched-H2+ single-execution release/example repair approved, screening remains maintenance | Apply all unchanged 18 release assertions to Example 41's one returned comparison while preserving its standalone TSV/summary; retain the frozen `12789/1285/1285` result and leave screening, release, registration, and citation decisions separate |
...
| `HP-PUBLIC-PAPER-CI-*` | Three-gate paper-aligned CI implemented; bounded PQS single-execution test repair approved | Preserve all three names/rows, PR behavior, path-aware routing, screening gate, and workflow bytes; remove only the duplicate Example 41 subprocess after its returned comparison feeds all 18 release checks |
