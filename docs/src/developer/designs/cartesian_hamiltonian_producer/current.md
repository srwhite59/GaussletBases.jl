...
| `HP-PUBLIC-ATOMIC-IDA-DOC-*` | Ten atomic-IDA inspection and tiny two-electron reference bindings documented; implementation and focused validation in maintenance | Preserve dense-reference/read-only semantics, the compact reference section, focused checks, and accepted undocumented-export count of `30`; remaining names are classified as six supported-documentation candidates, nineteen expert/experimental surfaces, and five future de-export audits, with none deletion-ready |
| `HP-PUBLIC-SUPPORTED-SURFACE-DOC-*` | Six classified supported-public bindings approved for bounded documentation | Move the bond-aligned concrete-type docstring, document the finite Coulomb representation, owner-specific metadata, narrow base facade, and strict external-GTO hashes; require `30 -> 24`, then stop with the nineteen expert/experimental and five audit candidates unchanged |
| `HP-PUBLIC-PAPER-CI-*` | Supported-floor `radial,misc` extension implemented and validated; maintenance | Preserve the exact Julia 1.10 `core,ida,cartesian,examples,radial,misc` selection, all three names/rows, and every other workflow/test behavior; leave `angular` pending a separate runtime/ownership audit |
