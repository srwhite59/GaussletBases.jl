...
| `HP-PUBLIC-RADIAL-PARITY-DOC-*` | Four coherent radial paper-parity exports approved for expert reproducibility documentation; not implemented | Add only bounded docstrings, one curated expert section, one recommended-route distinction, and focused docs checks; reduce undocumented exports exactly `16 -> 12` without changing the frozen artifact, cache, radial implementation, or Supported-floor selection |
