...
| `HP-PQS-PRF-*`, `HP-PQS-PARENT-GDIRECT-*` | Internal implementation and tests approved pending | Source-back consumer-selected parent residual blocks, exact parent one-body transforms, and direct-only onsite-calibrated Gaussian PRF-G/PRF-PRF blocks; keep selection, exchange, GTO-residual interactions, and Hamiltonian integration outside the lane |
...
The immediate producer-facing target is the approved internal PRF/direct
facility needed to continue the fixed-parent Cr2 contraction study without
turning its consumer selection policy into a repo default. In parallel, the
controlled Cr2 screened-Hartree off/on fixed-density comparison remains
authorized under the currently approved fitted-cloud reference convention.
Its numerical-complete H2/Be2 source and validation gates have passed:
...
2. **Parent-residual interaction boundary.** PRF mechanics and parent-backed
   Gaussian direct blocks are approved pending implementation. Transition-
   density exchange and PRF-to-GTO-residual blocks remain unresolved and must
   not be inferred from the direct resource.
