...
| `HP-PQS-DOCS-TAGDEPLOY-*` | Tag-aware Documenter deployment approved, pending implementation | Add fail-closed semantic-version tag classification, exact tag canonical paths, and standard prerelease exclusion from `stable` while preserving PR/main least privilege; no version, tag, or release is authorized |
