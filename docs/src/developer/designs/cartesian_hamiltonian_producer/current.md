...
| `HP-PUBLIC-DOCS-PROSE-CLEANUP-*` | Exact full-docs wording-lock cleanup approved for implementation | Change only `test/docs/runtests.jl`: preserve every classified mechanical policy/placement check and the shared public-surface owner, reduce four durable boundaries to headings, and delete descriptive phrase lists at exactly `717 -> 497`, `+19/-239`; stop on docs/source/workflow edits or net deletion below 200 lines |
