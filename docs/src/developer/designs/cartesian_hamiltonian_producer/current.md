...
| `HP-SOURCE-LAYOUT-RADIAL-ATOMIC-*` | Exact 14-file radial/atomic path-only relocation approved; implementation and validation pending | Move only the enumerated byte-identical files by `git mv`, substitute only their 14 root include paths in place, reconcile exact current path pointers, and require 14 complete renames; grant no file-body, include-order, test, module, API, numerical, workflow, release, or later-layout change |
