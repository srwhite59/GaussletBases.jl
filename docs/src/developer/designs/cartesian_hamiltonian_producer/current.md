...
| `HP-SLICE-HCHAIN-*` | Approved after complete numerical prototype | Land the validated compact G10 construction within the revised `850/900` source budget after replacing the finite quadratic tail scan and making its Vee transition diagnostic nontrivial; no solver or dense matrices |
