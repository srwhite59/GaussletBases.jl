...
| `HP-PUBLIC-STATUS-ONBOARDING-DOC-01` | Status/onboarding reconciliation approved; implementation pending | Shorten the eight assigned summaries around released v0.2 interfaces, standard PQS and radial-first onboarding; expose the two existing radial navigation entries, preserve scientific scope and stable/release policy, and use existing checks without test edits |
