...
| `HP-REP-XGTO-INTERCHANGE/PYSCF-EXPORT/CLOSESTDET-*` | Strict Cartesian-GTO reader, checkpoint-only PySCF exporter, frozen d-shell fixture, and explicit closest-determinant cleanup implemented; direct C2/aug-cc-pV6Z replay accepted | Maintain exact AO order, full source-overlap parity, unchanged raw import, explicit caller-thresholded cleanup, and checkpoint-only scope; add a reader-facing manual entry before any separately authorized RC2 decision, while basis-only/live-mean-field export and release actions remain outside v1 |
| `HP-REP-XGTO-READER-DOC-*` | Reader-facing interchange manual and focused documentation checks approved pending implementation | Add one bounded workflow page, Manual/index navigation, and curated API entries; preserve checkpoint attestation, explicit capture thresholds, unsupported-case boundaries, and a separate later RC2 decision |
