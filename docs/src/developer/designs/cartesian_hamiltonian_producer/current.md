...
| `HP-PQS-PUBLIC-RC2-*` | Exact `v0.2.0-rc2` candidate prepared at `2b3c23970`; clean validation completed; maintenance | Preserve the three distinct reader links, version/changelog identity, exact RC2/RC1/dev selectors, archive evidence, and absent `/stable/`; tag and GitHub-prerelease operations have separate lifecycle owners |
| `HP-PQS-PUBLIC-RC2-RELEASE-*` | One exact package-centered GitHub prerelease approved, pending publication | Publish only against immutable tag object `7c8a21b99`, with the exact `2,200`-byte body, prerelease/non-latest status, zero uploaded assets, and no tracked mutation |
| `HP-PQS-PUBLIC-RC2-TAG-*` | Immutable annotated tag and versioned documentation accepted; lifecycle closed | Preserve object `7c8a21b99`, frozen target `2b3c23970`, RC2/RC1/dev selector entries, and absent `/stable/`; release publication belongs only to the separate RC2 release record |
...
| `HP-PUBLIC-PAPER-CI-*` | Three-gate paper-aligned CI implemented/completed; maintenance | Preserve separate Julia `1.12` PQS and screening gates, the Julia `1.10` supported-floor gate, the `30`-minute job limit, exact public test owners and smokes, and unwired nested research suites |
