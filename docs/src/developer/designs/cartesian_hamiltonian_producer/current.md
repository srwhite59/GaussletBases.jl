...
| `HP-SOURCE-LAYOUT-FOUNDATION-*` | Exact 16-file foundation path-only relocation approved | Move the 16 reviewed owners into `src/foundation/`; preserve 13 byte-identical files, limit three others to four path-comment substitutions, update only exact include/test/current pointers, and stop on any executable, hash, order, or test deviation; no Step 4 authority |
...
| `HP-PUBLIC-PAPER-CI-*` | Supported-floor `radial,misc,angular_public` selection implemented and validated; maintenance | Preserve the exact Julia 1.10 `core,ida,cartesian,examples,radial,misc,angular_public` selection, all three names/rows, and exclusion of bare `angular` |
