...
| `HP-PUBLIC-PAPER-CI-*` | Existing three-gate CI remains implemented; bounded `docs_fast` source-push integrity extraction approved | Add only one shared mechanical public-surface owner and append `docs_fast` to Supported floor; preserve exact once-only docs/full-docs selection, the unchanged `fast` alias, all numerical groups, names/rows, classifier, paper/tag lanes, and every nonmechanical docs check |
