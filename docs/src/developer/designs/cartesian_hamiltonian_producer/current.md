...
| `HP-SOURCE-LAYOUT-CARTESIAN-*` | Exact 76-file Cartesian/PQS path-only relocation approved; implementation pending | Make the one authorized path-neutral docstring edit, then move the frozen inventory under `src/cartesian/` as R100 files relative to that prerequisite commit, substitute 40 root includes in place, and reconcile only classified current pointers; no executable body, test, workflow, API, numerical, follow-on, or release change |
