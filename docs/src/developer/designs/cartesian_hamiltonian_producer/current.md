...
| `HP-CARTESIAN-INTERNAL-MAINTENANCE-CI-*` | Four-suite workflow and screening deduplication implemented; timeout-only repair approved | Change only `timeout-minutes: 10 -> 15`, rerun manually, and require all four suites before closeout; preserve commands, cadence, permissions, process isolation, public CI, and the unscheduled occupied-first/represented-Hartree dispositions |
