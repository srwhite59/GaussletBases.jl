...
| `HP-PQS-READER-*` | Reader documentation and one public H2+ PQS/WL example approved; zero source/API delta | Add the bounded public page/example, expose the existing algorithm pages in navigation, and run the example in the existing Julia-1.10 quick-example group without importing private or paper-driver surfaces |
