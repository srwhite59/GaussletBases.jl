...
| `HP-REP-XGTO-INTERCHANGE/PYSCF-EXPORT/CLOSESTDET-*` | External Cartesian-GTO interchange approved, pending implementation | Add one strict TOML/Float64 bundle reader, one checkpoint-only optional PySCF exporter, and explicit full-rank closest-determinant cleanup while reusing the existing packet, overlap, and import owners; direct C2 replay must pass without a runtime permutation ledger, and inclusion in v0.2 requires a separately authorized new release candidate |
