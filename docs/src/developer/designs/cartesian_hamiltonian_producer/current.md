...
| `HP-PUBLIC-ANGULAR-PRODUCER-DOC-*` | Seven durable experimental angular profile/sequence bindings approved for documentation; not implemented | Add only bounded docstrings, one compact research-track explanation, one curated reference section, and focused docs checks; reduce undocumented exports exactly `23 -> 16`, keep `ShellLocalAngularProfileKey` exported but undocumented/unreferenced, and grant no de-export or workflow change |
