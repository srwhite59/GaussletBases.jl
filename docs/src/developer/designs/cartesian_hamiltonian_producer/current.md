...
| `HP-RETIRE-BYCENTER-SIDECAR-*` | Generic staged by-center sidecar retirement reauthorized after owner release; implementation pending | Delete only the orphaned generic carrier/closure and stale claims; preserve product-staged, factorized-final, dense-fallback, current PQS, and all numerical behavior; the archived high-order worktree remains physically retained and untouched behind its separate evidence-preservation gate |
