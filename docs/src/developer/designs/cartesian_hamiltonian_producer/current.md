...
| `HP-PQS-PUBLIC-RC1-*` | `v0.2.0-rc1` candidate preparation approved, pending implementation | Change only the package version and add one concise root changelog, then complete clean GitHub-install, public-example, release-test, archive, Julia `1.10`, and docs gates; no tag or release is authorized |
