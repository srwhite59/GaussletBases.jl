...
| `HP-PACKAGE-ROOT-PATH-*` | Package-root data/include indirection approved; implementation pending | Replace exactly five data paths and one nested include through one private root and helper; preserve all six targets and bytes, add no test or file move, and grant no later source-layout step |
