...
| `HP-REP-PQS-RG-WORKING-CI-*` | Public Cartesian CI extraction approved, pending implementation | Move bounded public result/overlap/import coverage into one `test/driver_public` owner under the existing `:cartesian` group with non-increasing tracked test LOC; retain unique private formula/artifact evidence unwired and add no CI row |
