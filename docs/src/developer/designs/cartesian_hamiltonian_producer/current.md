...
| `HP-SOURCE-LAYOUT-ORDINARY-*` | Exact 15-file Ordinary path-only relocation approved; implementation pending | Move only the frozen 14,430-line Ordinary inventory into `src/ordinary/` as byte-identical 100% renames, substitute 15 root include paths in place, and reconcile classified current pointers; no body, test, workflow, API, numerical, Cartesian/PQS, or later-layout change |
