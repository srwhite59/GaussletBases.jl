...
| `HP-PUBLIC-ATOMIC-IDA-DOC-*` | Ten atomic-IDA inspection and tiny two-electron reference bindings approved for documentation | Add only declaration docstrings, one compact existing reference-page section, and focused docs checks; require `40 -> 30`, preserve dense-reference/read-only semantics, and stop afterward for classification of the remaining exports |
