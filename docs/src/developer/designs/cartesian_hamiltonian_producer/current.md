...
| `HP-PQS-PUBLIC-*` | Matched-H2+ and supplied-field screened-Hartree public surfaces implemented; validation completed | Run the pending clean candidate replay from `Project.toml` by one isolated fresh resolution, archive its generated manifest and hashes, and preserve the frozen `12789/1285/1285` comparison; no release tag is yet authorized |
