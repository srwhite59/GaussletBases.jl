...
| `HP-PUBLIC-BASE-WORKING-DOC-*` | Retained expert staged Cartesian constructor approved for documentation; implementation pending | Add one docstring, one compact export-reference boundary, and focused docs checks; replace the count-only assertion with exact equality to the five reserved undocumented names while preserving unstable result-schema, private PRF, and no-second-facade limits |
