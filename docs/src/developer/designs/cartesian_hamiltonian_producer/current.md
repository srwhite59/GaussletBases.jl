...
| `HP-FN-03` / `HP-DRV-STAGE-FN-01` | Exact-order four-element terminal Gaussian-sum reduction approved under `HP-FN-03`; call-local buffers and staged producer remain maintenance | Replace only the 135-term scalar hot loop, preserve every element's term/arithmetic order and all endpoint facts, and require bitwise PQS/WL parity plus measured isolated and complete-comparison speedup without persistent state |
