...
| `HP-PUBLIC-FOUNDATION-DOC-*` | Foundational basis/mapping documentation approved; implementation pending | Document only the ten existing map-evaluation and basis-introspection generics in their declarations and existing reference page; require exact `71 -> 61` undocumented-export reduction without changing behavior or global docs policy |
