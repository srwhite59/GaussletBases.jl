...
| `HP-PUBLIC-PAPER-CI-*` | Three-gate paper-aligned CI implemented; documentation-only transition repair approved | Preserve all three names/rows and PR behavior; expand named no-op gates for proven docs-only pushes, instantiate the docs environment in the lightweight lane, retain fail-closed full routing otherwise, and leave performance/H2+ rewiring separate |
