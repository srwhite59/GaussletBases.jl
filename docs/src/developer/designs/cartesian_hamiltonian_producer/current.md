...
| --- | --- | --- |
| `HP-AUTHORITY-EXECUTION-WHITELIST-*` | Authority-preserving whole-file execution-whitelist relocation approved; implementation pending | Keep `authority.toml` sole authority and `registry.md` checked; replace only the marked `AGENTS.md` block with one generated whole-file view, stable fail-closed startup instructions, direct byte comparison, and existing adversarial failures; no schema, workflow, source/API, Step 4, or historical rewrite |
