...
| `HP-PUBLIC-EXPORT-INTEGRITY-*` | Four invalid export declarations confirmed; bounded removal and one core regression approved | Delete exactly two invalid root exports, one bare methodless generic, and two invalid internal exports with no replacement; preserve all numerical behavior and RC1/release state |
