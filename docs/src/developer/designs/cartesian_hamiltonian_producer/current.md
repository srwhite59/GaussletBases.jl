...
| `HP-SOURCE-LAYOUT-ANGULAR-*` | Exact five-file angular path-only relocation approved | Move only the five byte-identical angular owners into `src/angular/`, preserve their include positions and `_lanczos_ground_state_apply`, reconcile current paths, and run the full angular owner once plus the PQS release owner; no later layout step is authorized |
