...
| `HP-RETIRE-BYCENTER-SIDECAR-*` | Main retirement deferred; preflight completed and grants closed | Retain the generic carrier and `:staged_factorized` route because active `high-order/manager-lane` installs and validates them; require a separate high-order owner/migration decision and fresh cross-worktree caller audit before reconsidering deletion |
