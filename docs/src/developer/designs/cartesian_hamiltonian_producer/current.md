...
| `HP-RETIRE-CPB-PROVIDER-*` | Source retirement approved; validation pending | Delete only the orphaned 6,445-line provider and its root include; preserve CPB geometry, parent factors, raw blocks, pair materialization, and all current producer routes |
