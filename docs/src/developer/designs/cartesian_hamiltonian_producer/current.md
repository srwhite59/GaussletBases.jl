...
| `HP-PQS-DOCS-TAGDEPLOY-*` | Tag-aware Documenter deployment implemented; focused validation completed; maintenance | Preserve fail-closed semantic-version tag classification, exact main/tag canonical paths, standard prerelease exclusion from `stable`, and PR/main least privilege; no version, tag, or release is authorized |
...
| `HP-PQS-PUBLIC-RC1-TAG-*` | One annotated `v0.2.0-rc1` tag approved, pending creation | Tag only frozen target `1546c18d3`, push only that tag, and verify versioned docs without creating or advancing `stable`; no GitHub release or tracked-file mutation |
