...
| `HP-REP-PQS-RG-WORKING-CI-*` | Public Cartesian CI owner implemented/completed; maintenance | Preserve the `68`-line, `21`-check public result/overlap/import owner in the existing `:cartesian` group, net `-5` tracked test lines, and the unique unwired private formula/artifact evidence; add no CI row or duplicate nested coverage |
