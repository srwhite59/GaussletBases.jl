...
| `HP-PQS-DOCS-TAGDEPLOY-*` | Tag-aware deployment and explicit RC1 selector retention implemented; exact RC2 selector addition approved pending implementation | Preserve exact main/tag canonical paths, both bounded prerelease self-mappings, prerelease exclusion from the real `stable` alias, and PR/main least privilege |
...
| `HP-PQS-PUBLIC-RC2-*` | README parity and `v0.2.0-rc2` candidate preparation approved, not implemented | Add only the three distinct reader links, RC2 version/changelog section, exact RC2 selector, and focused checks; stop on any source, numerical, test-owner, workflow, dependency, fixture-format, or manifest-policy change |
