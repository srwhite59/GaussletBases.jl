...
| `HP-REP-PQS-RG-WORKING-CI-*` | Existing public Cartesian wiring implemented; external-import coverage extraction approved | Reuse the existing residual-GTO construction, move unique rotation/spin/metric/fingerprint checks into its public owner, delete the redundant first nested import testset, preserve the complete protected-sidecar testset, and require a negative tracked-test delta without a runner or CI change |
