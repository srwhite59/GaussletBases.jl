...
| `HP-RETIRE-BYCENTER-SIDECAR-*` | Unused generic staged by-center sidecar retirement approved; implementation pending | Delete only the uninstalled generic carrier, builders, branches, overload, and stale current claims with zero added source lines; preserve product-staged, factorized-final, dense-fallback, cache/storage, high-order, and numerical behavior; stop on any live caller or parity loss |
