...
| `HP-CARTESIAN-INTERNAL-MAINTENANCE-CI-*` | Weekly/manual four-suite internal gate and bounded screening deduplication approved | Add only the read-only Julia `1.12` scheduled workflow, run four existing suites in separate processes, delete only duplicates owned by stronger public/atomic tests, require a net-non-positive delta and one successful manual dispatch, and leave occupied-first and represented Hartree unscheduled |
