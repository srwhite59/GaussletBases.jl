...
| `HP-PQS-PUBLIC-MATCHED/SCREEN-*` | Matched-H2+ and supplied-field screened-Hartree public surfaces implemented; clean Julia `1.12.6` candidate replay completed | Preserve the archived fresh-resolution manifest and frozen `12789/1285/1285` comparison; release review and any version/tag decision remain separate |
| `HP-PQS-PUBLIC-COMPAT-*` | Exact six-bound root compatibility declaration implemented; validation completed | Preserve the declared ranges, Julia `1.10` floor, untracked-manifest policy, and fresh-resolution evidence contract; release review and any version/tag decision remain separate |
...
| `HP-PQS-DOCS-TAGDEPLOY-*` | Tag-aware deployment implemented; explicit RC1 selector repair approved pending implementation | Add only the `v0.2.0-rc1` self-mapping and focused test while preserving canonical paths, prerelease exclusion from `stable`, and PR/main least privilege; do not edit the workflow or tag |
| `HP-PQS-PUBLIC-RC1-*` | `v0.2.0-rc1` candidate prepared at `41fa897ae`; clean validation completed; maintenance | Preserve the exact candidate version and concise reader changelog; the tag operation is separately bounded below, while GitHub release publication, registration, citation metadata, and final-release decisions remain unauthorized |
| `HP-PQS-PUBLIC-RC1-TAG-*` | Immutable annotated tag published at object `a4284f0bf`; canonical RC1 docs live; selector-index acceptance pending | Preserve the tag and frozen target; close only after a `main` docs deployment lists RC1 while `/stable/` stays absent and `/dev/` remains intact; no GitHub release |
