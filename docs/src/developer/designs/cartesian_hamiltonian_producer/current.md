...
| `HP-PQS-PUBLIC-DOC-PARITY-*` | Bounded reader-document repair approved, not implemented; preflight found ten missing public docstrings | Add only those ten docstrings, the eleven-name curated reference, path-free provenance in two rendered pages, and the example-28 internal label; no executable source, API, numerical, citation, or release change |
