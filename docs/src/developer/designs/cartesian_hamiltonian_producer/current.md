...
| `HP-PUBLIC-QW-GEOMETRY-DOC-*` | Three experimental QW geometry-diagnostic bindings approved for documentation | Add only bounded docstrings, one compact operators/diagnostics reference section, an optional ordinary-branch cross-reference, and focused docs checks; distinguish nested source reuse from basis-overload construction and lightweight chain/square inspection from nested construction, require exact undocumented-export reduction `12 -> 9`, and grant no behavior, API, workflow, or namespace change |
