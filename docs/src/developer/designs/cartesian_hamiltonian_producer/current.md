...
| `HP-QW-NESTED-DIAT-*` | Repair approved for three existing root-exported ordinary-QW nested diatomic front doors | Restore the legal no-shared-shell packet, preserve actual endcap/panel provenance, and forward existing policy/`q`/`L` controls through diagnostics without changing defaults or current PQS/WL production |
