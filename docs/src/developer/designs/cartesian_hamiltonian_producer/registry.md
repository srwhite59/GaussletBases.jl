...
- **Owned paths:** none
- **Evidence:**
  - `git_commit`: `4b6e1c9dab7696a4a6b9f8198b225450de8924a9`
  - `manager_pass`: `638`
  - `git_commit`: `5a770a571d9527838aaaf540fed515cd03380b97`
  - `manager_pass`: `637`
- **Dependencies:** `HP-COLLINEAR-PQS-FN-01`
- **Scope:** Private Recursive Chain Retirement completed at 4b6e1c9dab7696a4a6b9f8198b225450de8924a9: exactly 708 deleted lines, zero additions in one file. Frozen retained-file and shared-helper hashes match. Square code, released interfaces, tests and historical evidence remain unchanged. No further execution grant.
...
- **Scope:** Private Recursive Chain Retirement validated by unchanged core, Cartesian 232/232 and collinear 222/222 owners, frozen hashes, package/helper checks and exact-head full CI 35136279056 plus Docs 35136278883. No test changes. Validation completed; no further execution or consumer grant.
...
- **Evidence:**
  ...
  - `git_commit`: `1dbc1856d7b699311dcf65ae6a68fb193f9c1009`
  - `manager_pass`: `649`
- **Dependencies:** none
- **Scope:** Maintain accepted relative damping/shifts in polynomial\_gaussian\_basic\_integral under Basic Gaussian Integral Arithmetic Repair. Preserve accepted arguments, absolute moments, errors and callers. The implementation grant is consumed; no successor is authorized. Exclude other kernels, helpers, residual algorithms, q9 loading/requalification, numerical-policy changes, workflows and releases.
...
- **Scope:** Maintain the accepted 35-line, 81-check core regression under Basic Gaussian Integral Arithmetic Repair: independent translated primitive/moment, extra-Gaussian, kinetic and accepted-input checks. The implementation grant is consumed. Preserve references, tolerances and caller gates; exclude new owners/frameworks, q9 audit replay, physical qualification and workflow edits.
...
- **Execution whitelist:** `true`
- **Documents:**
  - `canonical` [coulomb\_accuracy\_policy.md](coulomb_accuracy_policy.md); heading `Displaced Gaussian Arithmetic Repair`
- **Owned paths:**
  - `source` / `existing`: `src/foundation/GaussianAnalyticIntegrals.jl`
...
- **Evidence:**
  - `manager_pass`: `623`
- **Dependencies:** none
- **Scope:** Maintain compact/high behavior under Coulomb Accuracy Policy, Standard60 Deferral. Standard60 remains unimplemented, deferred pending a named consumer and demonstrated accuracy/cost benefit. Preserve exact parameters/fingerprint and recovered evidence. Demonstrated kernel discrepancies are closed; diffuse/long-range limits remain. No implementation, provenance/driver additions, default change, retuning, or successor packet.
...
- **Evidence:**
  - `manager_pass`: `623`
- **Dependencies:** none
- **Scope:** Maintain existing compact/high tests under Coulomb Accuracy Policy, Standard60 Deferral. Completion does not certify unimplemented standard60. Preserve existing assertions and numerical policy; no new standard60, provenance, driver, or kernel-repair tests, implementation grant, or successor packet. Reconsider only after a named consumer demonstrates accuracy/cost benefit under separate authority.
...
- **Execution whitelist:** `true`
- **Documents:**
  - `canonical` [pqs\_public\_surface.md](pqs_public_surface.md); heading `Atomic IDA Inspection And Tiny Two-Electron Reference Documentation`
- **Owned paths:**
...
### HP-PUBLIC-CARTESIAN-FRONTDOOR-DOC-FN-01 - maintain the public Cartesian front-door documentation
...
- **Scope:** Maintain the accepted front-door documentation under the canonical Public Cartesian Front-Door Documentation heading: facade schemas/defaults/geometry, supported \`q\`/\`ns\`, atom-only \`d\`, corrected Export example, PQS snippet, and \`Vee\` mapping. Preserve Example 39 as the numerical owner and the provenance spelling. No behavior, API, workflow, artifact, or release changes.
...
- **Scope:** Maintain the accepted structural checks in \`test/docs/runtests.jl\` under the canonical Public Cartesian Front-Door Documentation heading: parsed snippet schema/call, Example 39 inputs, corrected \`d\`, \`q\`/\`ns\`, geometry, and \`Vee\` identities. Construct no Hamiltonian and add no parser framework, numerical assertion, workflow, API, metadata, artifact, or release policy.
...
- **Owned paths:**
  ...
  - `source` / `existing`: `src/GaussletBases.jl`
- **Evidence:**
  - `git_commit`: `312f69a4b757e8d559387e0ef14450f031691039`
  - `manager_pass`: `556`
  - `manager_pass`: `557`
- **Dependencies:** `HP-PUBLIC-EXPORT-INTEGRITY-FN-01`, `HP-PUBLIC-SUPPORTED-SURFACE-DOC-FN-01`, `HP-QW-NESTED-DIAT-FN-01`
- **Scope:** Maintain only the accepted expert/experimental bond-aligned geometry documentation from commit 312f69a4b757e8d559387e0ef14450f031691039: the 18-line docstring on the existing bond\_aligned\_diatomic\_geometry\_payload generic declaration in src/GaussletBases.jl and the 21-line Expert/experimental bond-aligned geometry inspection section in docs/src/reference/bases\_and\_mappings.md. Keep the central function curated with the already-documented exported BondAlignedDiatomicGeometryPoint3D, BondAlignedDiatomicGeometryNucleus3D, BondAlignedDiatomicGeometryBox3D, BondAlignedDiatomicGeometryPayload3D, and BondAlignedDiatomicGeometryPlaneSlice3D records plus bond\_aligned\_diatomic\_source\_geometry\_payload and bond\_aligned\_diatomic\_plane\_slice, without duplicating their docstrings. Preserve the inspection/visualization-only contract: the function neither constructs nor mutates a basis or Hamiltonian; overloads cover the supported bond-aligned basis, ordinary operators, nested source/fixed-block combinations, and residual-augmented operators; nested payloads describe compressed fixed centers while the source payload separately describes raw source-region points; multi-object inputs share one basis geometry where current methods require it; and returned records/vectors are read-only inspection data rather than mutable construction state. Keep the interface tied to current bond-aligned QW/nested geometry and provenance conventions, not a general molecular-geometry API, plotting framework, or stable serialization format. Preserve every implementation, method, dispatch, field, payload value, export, numerical behavior, plotting dependency, serialization behavior, ordinary test, example, global documentation policy, compatibility floor, workflow, version, tag, and release artifact. Reserve but do not authorize two next-minor namespace transactions: OneCenterAtomicNestedStructureDiagnostics with one\_center\_atomic\_nested\_structure\_diagnostics and one\_center\_atomic\_nested\_structure\_report; and diagnose\_qwrg\_residual\_space with the already-documented QWRGResidualSpaceDiagnostics type. Retain every implementation for qualified internal and research use. These reservations are namespace reductions, not source deletion or performance work; they authorize no de-export, migration edit, v0.2.x compatibility change, version, changelog, release, or v0.3 development.
...
- **Scope:** Maintain only the accepted 13-line focused check addition from commit 312f69a4b757e8d559387e0ef14450f031691039 in test/docs/runtests.jl. Require bond\_aligned\_diatomic\_geometry\_payload and its exact seven existing companion record/operation bindings to remain exported, carry Julia documentation, and appear in the Expert/experimental bond-aligned geometry inspection section of docs/src/reference/bases\_and\_mappings.md. Preserve the inspection-only, overload-family, compressed-versus-raw-source, same-geometry, non-general-API, non-plotting-framework, non-serialization, and read-only-inspection-data limits. Preserve the accepted undocumented exported-binding backlog of 23, excluding the module self-binding, and the remaining classification as nineteen undocumented expert/experimental bindings plus four original de-export candidates, with the already-documented QWRGResidualSpaceDiagnostics joined to the future diagnose\_qwrg\_residual\_space namespace transaction. Use Documenter as the executable @docs resolution gate. Add no global undocumented-name inventory or allowlist, checkdocs change, parser framework, test file, numerical assertion, fixture, workflow, source behavior, ordinary-test change, export change, migration edit, version, changelog, release, or broader reference-page scan.
...
- **Execution whitelist:** `true`
- **Documents:**
  - `canonical` [pqs\_public\_surface.md](pqs_public_surface.md); heading `Partition Hierarchy And Leaf-Local Accessor Documentation`
- **Owned paths:**
  - `docs` / `existing`: `docs/src/reference/bases_and_mappings.md`
  - `source` / `existing`: `src/GaussletBases.jl`
- **Evidence:**
...
- **Execution whitelist:** `true`
- **Documents:**
  - `canonical` [pqs\_public\_surface.md](pqs_public_surface.md); heading `Radial Paper-Parity Reproducibility Documentation`
- **Owned paths:**
...
- **Execution whitelist:** `true`
- **Documents:**
  - `canonical` [r1\_public\_base\_producer.md](r1_public_base_producer.md); heading `Reference Discoverability Completion`
...
- **Execution whitelist:** `true`
- **Documents:**
  - `canonical` [pqs\_public\_surface.md](pqs_public_surface.md); heading `Supported Public Surface Documentation`
- **Owned paths:**
  - `docs` / `existing`: `docs/src/reference/bases_and_mappings.md`
  - `docs` / `existing`: `docs/src/reference/export.md`
  - `docs` / `existing`: `docs/src/reference/operators_and_diagnostics.md`
...
- **Evidence:**
  - `manager_pass`: `621`
  - `git_commit`: `a7ec77247b9bc45a349771e58cb76cb8019e08f8`
  - `manager_pass`: `622`
- **Dependencies:** none
- **Scope:** Maintain the accepted foundation bases/quadrature repair under Radial Construction And Quadrature Convergence. Preserve five construction attempts, early exit, explicit controls, truthful warnings, quadrature schedules/tolerances, and last-grid return. No implementation grant remains. Exclude near-origin accuracy changes, other numerical findings, presets, stored metadata, frameworks, APIs, workflows, and releases; standard60 remains held.
...
- **Scope:** Maintain accepted core/radial coverage under Radial Construction And Quadrature Convergence: construction exits, fifth-candidate equality, explicit controls, truthful quadrature diagnostics, and two single-warning expectations. Preserve numerical assertions and existing owners. No implementation grant, new file/framework, large sweep, workflow, or unrelated test change.
...
- **Evidence:**
  ...
  - `git_commit`: `f13e946dfffd47ca9bc06e003f76f54c25316415`
  - `manager_pass`: `574`
  - `manager_pass`: `575`
  - `manager_pass`: `576`
  - `manager_pass`: `577`
- **Dependencies:** none
- **Scope:** The orphaned generic staged by-center sidecar carrier, default-range/builder/attach helpers, generic selector and :staged\_factorized path branches, matching generic nuclear overload, and two stale current-documentation claims were removed in commit f13e946dfffd47ca9bc06e003f76f54c25316415. The accepted implementation deleted 210 source lines with zero additions and added no replacement, API, cache, metadata, helper, test, file, dependency, workflow, version, or release change. Fixed-block storage/cache, every product-staged owner and method, nonzero-row and contraction kernels, factorized-final and dense routes, current PQS behavior, and numerical results remain intact. The archived high-order worktree and its ignored evidence remain physically preserved behind their separate archive gate and carry no mainline compatibility claim.
...
- **Evidence:**
  ...
  - `manager_pass`: `577`
- **Dependencies:** none
- **Scope:** Unchanged existing owners and an uncommitted baseline/candidate probe established exact ordinary factorized-final and product-staged endcap-panel parity for every per-center matrix, summed nuclear matrix, kinetic matrix, coefficients, and complete H1. Dimensions remained 433 and 397, route identities were unchanged, and the product-staged dense-oracle error was 8.88e-16. Core, public Cartesian 232/232, nested PQS 464/464 plus supplemented facade 64/64, matched H2+ 18/18, docs 157/157 plus 10/10, package load, authority checks, Documenter, CI run 33664855002, and Docs run 33664855006 passed. No committed test, assertion, fixture, runner, workflow, helper, or probe changed, and the archived high-order worktree was not run or modified.
...
- **Owned paths:** none
- **Evidence:**
  - `git_commit`: `8fcda0086f73dbd1348aa6b261c7862f1cf64bb3`
  - `manager_pass`: `573`
- **Dependencies:** none
- **Scope:** The inert projected-q-shell staged descriptor closure, its construction and descriptor-only diagnostics/provenance, and the two-line pqs\_staged\_unit\_descriptor propagation were deleted without replacement. The accepted two-file implementation removed 593 and added 2 source lines while retaining byte-exact boundary-mode selection, raw-boundary projection, symmetric Lowdin cleanup, coefficient matrix, support indices/states, metric packet, actual layer, consumed diagnostics/provenance, ordinary and mapped behavior, and all public results. No replacement carrier, NamedTuple, accessor, alias, deprecation, serialization, copied diagnostic, status vocabulary, helper, test, file, API, cache, dependency, workflow, numerical policy, version, or release change was added. The generic by-center sidecar, old XY/shell-plus-core/hierarchical milestones, active product-staged sidecars, factorized/support-reference kernels, and high-order branch remain outside this retired transaction.
...
- **Execution whitelist:** `false`
- **Documents:**
  - `evidence` [common\_terminal\_shell\_decomposition.md](common_terminal_shell_decomposition.md); heading `Projected-Q-Shell Staged Descriptor Retirement`
- **Owned paths:** none
- **Evidence:**
  - `git_commit`: `8fcda0086f73dbd1348aa6b261c7862f1cf64bb3`
  - `manager_pass`: `573`
- **Dependencies:** none
- **Scope:** The unchanged existing owners and transient baseline/candidate probes established byte-identical projected-q-shell coefficients, metric packets, ordinary-QW H2 matrices, and packet/basis fingerprints, plus unchanged matched H2+ dimensions, topology, energies, captures, residuals, warnings, and accounting. Warm representative-layer and matched-comparison measurements showed no material regression. No test, fixture, golden value, assertion, runner, workflow, helper, or committed probe was added or edited.
