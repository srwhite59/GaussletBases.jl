...
- This live volume begins with Pass 430. Pass entries are preserved in accepted
...
New source authority remains design-manager-owned even when the user requests
...
## Cartesian Hamiltonian Producer Pass 450 - Accept Consumer-Callable PRFs

Commit(s):
- `be59362b8` - bind each PRF to the exact source block named by its descriptor.
- this commit - close lifecycle and rotate the bounded manager ledger.

Summary:
- Both additive and fixed-span composition now reject a descriptor/PRF pair
  unless source key, support indices, and support states identify the same
  terminal block. The check occurs once immediately after descriptor
  validation and before either composition branch.
- The design-manager-ratified seven-export, two-type interface is accepted as
  an opt-in expert API. It changes no ordinary PQS default and still requires
  consumers to supply every target, shell choice, orientation, and mode count.
- Passes 407-429 moved verbatim into a `927`-line archive with SHA-256
  `83075d2d43762538fcad502a075cae85f03024f01a6a08e911b4b3f03ad42d12`;
  the live ledger retains Passes 430-450.

Goal advancement / guardrail:
- MT4 completes producer-side PRF exposure needed by Hooke. MT5 records the
  corrected role boundary: user intent does not let repo-manager create new
  design authority; Pass 449 design-manager ratification governed this fix.

Carrying-cost accounting:
- deleted: no numerical path; `927` startup-ledger lines moved to history.
- simplified: one shared six-line binding check covers both composition modes.
- quarantined: Be target selection and energy comparisons remain consumer work.
- not deleted because: established low-level PRF/injection/operator/interaction
  owners remain the implementation behind the expert API.
- exact remaining caller/blocker: Hooke must supply and validate explicit Be
  `1s/2s` parent targets; no producer plumbing blocker remains.
- added/deleted `src` lines: `6/0`; new tests: one assertion in the existing H2
  owner, no new file.
- new metadata/status fields: none.
- validation: package load, H2 `464/464`, supplemented facade `73/73`, exact
  readback parity, terminal due diligence, anti-bloat review, archive hash,
  authority/docs checks, manager-log bound, and `git diff --check`.

### Medium-Term Goal Checkpoint After Pass 450

- **MT1 conformance remediation - active.** This fix closes the PRF consumer
  binding defect only; remaining Pass 398 items stay independent.
- **MT2 controlled Cr2 source migration - completed.** No Cr2 basis,
  Hamiltonian, solver, or accepted result changed.
- **MT3 approved producer facilities - active.** Charged sectors and matched
  paper mechanics are implemented; Standard60/driver exposure remains pending.
- **MT4 residual/protected evidence - active/refined.** Consumer-callable PRFs
  are accepted. Hooke owns the next Be target/energy study; exchange and
  PRF-GTO interaction questions remain separate.
- **MT5 authority maintenance - maintenance.** Design-manager owns new grants;
