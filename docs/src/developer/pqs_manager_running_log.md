...
- This live volume begins with Pass 496. Pass entries are preserved in accepted
...
## Cartesian Hamiltonian Producer Pass 516 - Accept External Cartesian GTO Interchange

Commit(s):
- `bd1d11cec71c2257b311054bb064e361bb533042` - reader, exporter,
  closest-determinant operation, and frozen fixture gate.
- `0ca4e18c7ff550ec6f254f63b9f075193094ccd0` - exact exporter-driven fixture
  regeneration and exporter-version assertion.
- this docs-only lifecycle closeout and bounded-ledger rotation.

Summary:
- Accepted the strict v1 TOML/Float64 interchange, two public Julia operations,
  optional checkpoint-only PySCF exporter, and existing packet/import reuse.
  The regenerated H2/cc-pVTZ fixture retained byte-identical AO, overlap, MO,
  and payload data; its payload SHA-256 is `de579e9188f46d7e60cb6c7a0ad86263ad66edc22d5a905a64a4129d2669b0a1`.
- The read-only C2/aug-cc-pV6Z replay used `588` Cartesian AOs and six occupied
  orbitals in a `2071+48=2119` basis. Source-metric error was `1.78e-15`, the
  occupied Gram range was `0.9999727179464206` to `0.9999999974932495`, prior
  accepted extrema agreed within `3.56e-15`, and cleaned orthogonality error
  was `3.77e-15`. No runtime permutation ledger was used.

Goal advancement / guardrail:
- MT7 moves to completed/maintenance. This validates interchange, not C2
  energy or production policy. Basis-only/live-mean-field export remains
  outside v1. A reader-facing manual entry and a separately authorized release
  candidate remain prerequisites for v0.2 inclusion.

Carrying-cost accounting:
- deleted: no repository code; the accepted direct route makes the external
  runtime permutation ledger unnecessary for continuing use.
- simplified: checkpoint-to-packet normalization and ordering now have one
  reusable source-backed path.
- quarantined: large C2 packet/snapshots and historical ledger remain external
  evidence only.
- not deleted because: the existing packet, overlap, and raw importer remain
  the canonical transfer owners.
- exact remaining caller/blocker: none for v1 mechanics; reader documentation
  and any RC2 lifecycle are separate work.
- added/deleted `src` lines: `383/0`; optional exporter: `245` lines; new
  tests: one `93`-line extension and one `17,171`-byte logical fixture, with no
  new test owner; new
  metadata/status fields: none.
- validation: implementation and budget review, fixture regeneration `80/80`,
