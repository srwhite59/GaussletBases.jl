...
- This live volume begins with Pass 451. Pass entries are preserved in accepted
...
## Cartesian Hamiltonian Producer Pass 472 - Authorize Ordinary-QW Diatomic Front-Door Repair

Commit(s):
- this docs-only design authority amendment.

Summary:
- Selected repair rather than retirement for the three root-exported
  bond-aligned ordinary-QW nested source, fixed-block, and diagnostics front
  doors. `STATUS.md`, the rendered API reference, and algorithm pages present
  them as current supported experimental workflows; withdrawing those claims
  would require a separate intentional compatibility decision.
- `HP-QW-NESTED-DIAT-FN-01/TEST-01` authorize only three demonstrated fixes:
  build the required packet for a legal unsplit/no-shared-layer source,
  preserve actual endcap/panel provenance without concrete-type coercion, and
  forward the existing policy/`q`/`L` controls through basis diagnostics.

Goal advancement / guardrail:
- MT1 gains a bounded exported-surface correctness repair. Current PQS/WL
  source-box production, ordinary-QW numerical policy, defaults, coefficients,
  dimensions, shell geometry, residuals, corrections, chain/square routes,
  artifacts, and solvers remain unchanged.
- Implementation is limited to two existing source files, preferred/hard
  `30/60` added source lines, and two regressions in `test/core/runtests.jl`
  within preferred/hard `45/70` added test lines. No new file, type, field,
  helper, keyword, export, metadata, adapter, or fallback is approved.

Carrying-cost accounting:
- deleted: no source in this docs-only pass; Passes 430-450 moved verbatim to a
  `972`-line task-gated archive.
- simplified: one explicit repair contract replaces ambiguous supported-but-
  unowned status.
- quarantined: historical nested suites and the deleted PQS fallback remain
  outside implementation authority.
- not deleted because: three root exports retain explicit current support and
  validated-workflow documentation.
- exact remaining caller/blocker: implement the three bounded corrections and
  pass unsplit-H2 plus endcap diagnostics regressions before lifecycle closure.
- added/deleted `src` lines: `0/0`; new tests: none in this docs-only pass; new
  metadata/status fields: none.
