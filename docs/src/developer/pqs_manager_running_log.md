...
- This live volume begins with Pass 592. Pass entries are preserved in accepted
...
## Cartesian Hamiltonian Producer Pass 612 - Authorize Review Residuals And Rotate Ledger

Rotated 753 accepted lines (Passes 567-591) verbatim before this entry; retained
the latest 20 passes and refreshed stale strategic summaries. The archive hash
is recorded above. This is strategic compression, not a rewrite of history.
MT1/MT6 now have one reader-review residual packet: four screening tolerances,
the unused 70-line explanation page, and existing example-guide classifications.
Source inspection additionally caught Example 38's reversed documented defaults:
MWG is current default; nearest/GGT is opt-in fallback/debug. No numerical defect
was found or inferred. Examples 26-28 are manual diagnostics; 29/30/34-37 are
historical internal regressions; 31/32/33 are supported demonstrations; 38 is an
expert manual reference. Lack of script CI is not lack of maintenance.

`HP-PUBLIC-REVIEW-RESIDUAL-DOC-01` receives docs-only design authority, with
35/50 preferred/hard reader additions and net reduction required. Deleted:
only the duplicate page is authorized; simplified: missing-default and script
guidance; quarantined: new scheduling, source changes, and stable publication.
Existing developer architecture and Example 41's shared release owner stay live.
Source +0/-0; no test, helper, metadata/status field, or numerical run. Validation
uses direct source/link inspection, existing docs/public-surface checks,
authority/self-test, deterministic views, package load, Documenter, log bound,
