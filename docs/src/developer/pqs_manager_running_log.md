...
- This live volume begins with Pass 475. Pass entries are preserved in accepted
...
## Cartesian Hamiltonian Producer Pass 495 - Authorize RC1 Selector Repair After Immutable Tag

Commit(s):
- tag object `a4284f0bf448fb9d717de26ccbe1e9fc16db5ed2` - published annotated
  `v0.2.0-rc1`, peeling to frozen target `1546c18d3058cce2b5051b50788cda3c12585e51`;
- this docs-only authority amendment.

Summary:
- Accepted the tag mutation itself as complete and removed its execution grant.
  Tag-triggered Docs run `32295705338` passed, the exact canonical RC1 folder
  is live, `/stable/` remains absent, and `/dev/` is unchanged. The tag
  validation record stays open only because `versions.js` omitted RC1.
- Installed Documenter `1.17.0` source confirms the cause: default release
  expansion filters prereleases, while an explicit
  `"v0.2.0-rc1" => "v0.2.0-rc1"` entry retains the folder in the selector
  without producing a self-symlink or making it eligible for `stable`.
- Existing `HP-PQS-DOCS-TAGDEPLOY-FN-01/TEST-01` now authorize only that
  self-mapping in `docs/make.jl` and focused isolated expansion coverage in
  `test/docs/runtests.jl`. The workflow, tag, package, and release state are
  immutable in this repair.

Goal advancement / guardrail:
- MT5 advances from tag publication to one bounded deployed-index correction.
  The implementation must fit within `8` added builder lines and `25` added
  existing-test lines, then a `main` deployment must list RC1 while preserving
  absent `stable` and intact `dev`. Failure returns to design-manager; it is
  never repaired by moving, deleting, or recreating the tag.

Carrying-cost accounting:
- deleted: further tag-mutation permission and stale pre-tag status.
- simplified: one explicit self-mapping replaces any dynamic prerelease-index
  policy.
- quarantined: GitHub release, registration, citation, final release, custom
  aliases, arbitrary prereleases, and workflow/credential changes.
- not deleted because: standard final-release selectors and the existing
  least-privilege deployment path remain correct.
- exact remaining caller/blocker: repo-manager must implement the two-file
  selector repair, obtain a successful `main` deployment, and return live
  `versions.js`, `stable`, `dev`, and RC1 evidence for lifecycle closeout.
- added/deleted `src` lines: `0/0`; new tests: none in this design pass; new
  metadata/status fields: none.
- validation: synchronized HEAD/origin and dirty-state review, immutable tag
  object/peel/tree evidence, installed Documenter expansion audit, exact
  authority/path/status scans, authority render/check/self-test, docs tests,
  Documenter, manager-log bound, docs-only scope review, and
  `git diff --check`.

### Medium-Term Goal Checkpoint After Pass 495
...
- **MT4 - active:** residual/protected and consumer-owned PRF studies are
  unchanged.
- **MT5 - active/refined:** the RC1 candidate and immutable tag are published;
  selector-index repair is the sole remaining tag-acceptance blocker. GitHub
  release publication remains separately unauthorized.
- **MT6 - active:** Passes 451-474 moved verbatim to a `952`-line archive with
  SHA-256 `e07d17fd739f8519511e79dca4ee994a7bf2a8bc61c7703ce147fca18d353169`;
  the live ledger retains Passes 475-495 and stays below its hard bound.

### Strategic Compression After Pass 495

- Schema-v3 machine authority, generated views, and bounded ledgers remain the
  stable governance base; no broad documentation migration is active.
- The v0.2 public PQS/Hartree surface, dependency ranges, reader parity,
  candidate preparation, and immutable RC1 tag have crossed their numerical
  and deployment gates. The only open RC1 tag check is selector visibility.
- Large obsolete Cartesian planning/provider ladders and unsupported PRF root
